...
self.addEventListener("fetch",e=>{
 ...
 if(/\.(?:js|css)$/.test(u.pathname)){e.respondWith(fetch(e.request,{cache:"reload"}).then(resp=>{if(resp.ok){const cp=resp.clone();caches.open(C).then(c=>c.put(e.request,cp))}return resp}).catch(()=>caches.match(e.request)));return}
