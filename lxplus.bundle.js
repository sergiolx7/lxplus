...
/* ===== lxplus.js ===== */
window.__LX_JS_BUILD='R12.3-NEXUS-20260924';

/* ===== config.js · LX Plus v25.50 ===== */
window.LX=window.LX||{};
LX.config={
  version:'R12.3-NEXUS-20260924',
...
/* ===== lx-drive-storage.js · LX Plus R12.1 =====
   Drive playback requires a verified R12 Worker and an authenticated byte stream. */
(()=>{
 const LX=window.LX=window.LX||{};
 ...
 let healthCache={endpoint:'',at:0,status:null};
 function endpoint(){
  ...
  return global||local||baked||'';
 }
 function configured(){return /^https:\/\//i.test(endpoint())}
 function streamUrl(fileId,resourceKey=''){const ep=endpoint(),id=String(fileId||'').trim(),rk=String(resourceKey||'').trim();return ep&&id?`${ep}/v/${encodeURIComponent(id)}${rk?`?resourcekey=${encodeURIComponent(rk)}`:''}`:''}
 function probeUrl(fileId,resourceKey=''){const ep=endpoint(),id=String(fileId||'').trim(),rk=String(resourceKey||'').trim();return ep&&id?`${ep}/probe/${encodeURIComponent(id)}${rk?`?resourcekey=${encodeURIComponent(rk)}`:''}`:''}
 function deployUrl(){
 ...
 function ref(value){const raw=String(value||'').trim();if(/^gdrive:/i.test(raw)){const [id,query='']=raw.slice(7).split('?');return {id,resourceKey:new URLSearchParams(query).get('resourcekey')||''}}try{const u=new URL(raw);if(!/(^|\.)drive\.google\.com$/i.test(u.hostname))return {id:'',resourceKey:''};return {id:(u.pathname.match(/\/(?:file\/)?d\/([A-Za-z0-9_-]{10,})/)||[])[1]||u.searchParams.get('id')||'',resourceKey:u.searchParams.get('resourcekey')||''}}catch{return {id:'',resourceKey:''}}}
 function explain(code){code=String(code||'');const messages={LX_STORAGE_ENDPOINT_MISSING:'O endereço do LX Storage não foi configurado no ADM.',LX_STORAGE_ENDPOINT_INVALID:'O endereço do LX Storage deve ser uma URL HTTPS do Worker.',LX_STORAGE_WORKER_OUTDATED:'O endereço salvo aponta para um Worker antigo. Implante o Worker R12 e salve a nova URL no ADM → Mídia & Upload.',LX_STORAGE_NETWORK:'O navegador não conseguiu acessar o Worker. Confira a URL, o CORS do domínio e a conexão.',LX_STORAGE_TIMEOUT:'O Worker demorou demais para responder. Teste novamente e confira os logs da Cloudflare.',DRIVE_AUTH_NOT_CONFIGURED:'A conta do Google Drive ainda não foi configurada nos Secrets do Worker.',DRIVE_AUTH_INVALID:'O Google recusou a autenticação. Confira GOOGLE_CLIENT_EMAIL e GOOGLE_PRIVATE_KEY no Worker.',DRIVE_PERMISSION_OR_QUOTA:'O Google recusou o acesso. Confira a API do Drive, a cota e o acesso da pasta à conta de serviço.',DRIVE_API_DISABLED:'Ative a Google Drive API no projeto da conta de serviço e teste a conexão novamente.',DRIVE_API_REJECTED:'A API do Drive recusou o teste. Confira no Google Cloud as restrições da API e os logs do Worker.',DRIVE_DOMAIN_POLICY:'A política da sua conta Google bloqueou o acesso ao Drive pela conta de serviço.',DRIVE_QUOTA_EXCEEDED:'A API do Drive atingiu o limite de uso. Confira as cotas do projeto no Google Cloud.',FILE_NOT_FOUND_OR_NO_ACCESS:'Arquivo não encontrado ou sem acesso para a conta de serviço.',DRIVE_DOWNLOAD_DISABLED:'O proprietário bloqueou o download deste arquivo.',DRIVE_RANGE_UNAVAILABLE:'O Drive não entregou Range/206 para este arquivo.',DRIVE_RETURNED_NON_MEDIA:'O Google respondeu com HTML ou JSON em vez de bytes de vídeo.',NOT_A_MEDIA_FILE:'O link não aponta para um arquivo de vídeo.',DRIVE_FORMAT_INCOMPATIBLE:'Formato incompatível com o navegador. Use MP4 H.264 + AAC.',PUBLIC_FALLBACK_UNAVAILABLE:'O fallback público não entregou bytes de vídeo válidos.',GDRIVE_LINK_INVALID:'Não reconheci o ID do arquivo nesse link do Drive.'};if(messages[code])return messages[code];const status=code.match(/^(?:LX_STORAGE_HTTP_|DRIVE_HTTP_)(\d{3})$/);if(status)return `O LX Storage respondeu HTTP ${status[1]}. Teste a conexão no ADM e confira os logs do Worker.`;return code&&/^[A-Z][A-Z0-9_]{2,80}$/.test(code)?`LX Storage: ${code}. Confira o diagnóstico no ADM.`:'Falha na reprodução. Teste o LX Storage no ADM para localizar a causa.'}
 async function fetchJson(url){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),9000);try{const response=await fetch(url,{cache:'no-store',credentials:'omit',signal:controller.signal});let data=null;try{data=await response.json()}catch{}return {response,data}}catch(error){throw new Error(error?.name==='AbortError'?'LX_STORAGE_TIMEOUT':'LX_STORAGE_NETWORK')}finally{clearTimeout(timer)}}
 async function test(ep=endpoint(),force=false){
  ep=clean(ep);if(!ep)throw new Error('LX_STORAGE_ENDPOINT_MISSING');if(!/^https:\/\//i.test(ep))throw new Error('LX_STORAGE_ENDPOINT_INVALID');
  if(!force&&healthCache.endpoint===ep&&healthCache.status&&Date.now()-healthCache.at<30000)return healthCache.status;
  const {response,data}=await fetchJson(`${ep}/health?check=1`);
  if(!response.ok)throw new Error(`LX_STORAGE_HTTP_${response.status}`);
  if(data?.service!=='LX Storage'||data?.version!=='R12')throw new Error('LX_STORAGE_WORKER_OUTDATED');
  const status={ok:true,endpoint:ep,googleConfigured:data.googleConfigured===true,googleConnected:data.googleConnected===true,mode:data.mode||'unconfigured',error:data.error||'',service:data.service,version:data.version};
  healthCache={endpoint:ep,at:Date.now(),status};return status;
 }
 async function ensureReady(force=false){const status=await test(endpoint(),force);if(status.googleConnected||status.mode==='public-fallback')return status;throw new Error(status.error||(!status.googleConfigured?'DRIVE_AUTH_NOT_CONFIGURED':'DRIVE_AUTH_INVALID'))}
 async function probe(value){const {id,resourceKey}=ref(value);if(!/^[A-Za-z0-9_-]{10,}$/.test(id))throw new Error('GDRIVE_LINK_INVALID');await ensureReady();const url=probeUrl(id,resourceKey);const {response,data}=await fetchJson(url);if(!response.ok||!data?.ok)throw new Error(data?.error||`LX_STORAGE_HTTP_${response.status}`);return data}
 async function saveEndpoint(ep,{global=true}={}){
  ...
  healthCache={endpoint:'',at:0,status:null};
  if(global&&LX.data?.saveBranding){const b=LX.data.branding?.()||{};await LX.data.saveBranding({...b,driveStorageEndpoint:ep})}
  return ep;
 }
 LX.driveStorage={endpoint,configured,streamUrl,probeUrl,probe,ref,explain,test,ensureReady,saveEndpoint,deployUrl,openAutoDeploy,workerRepo:WORKER_REPO};
})();
window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['drive-storage']='R12';
...
(()=>{
 ...
 const driveRef=v=>{v=clean(v);let id='',resourceKey='';if(/^gdrive:/i.test(v)){const tail=v.slice(7),q=tail.indexOf('?');id=(q>=0?tail.slice(0,q):tail).trim();try{const sp=new URLSearchParams(q>=0?tail.slice(q+1):'');resourceKey=sp.get('resourcekey')||''}catch{}}else{const a=v.match(/\/file\/d\/([a-zA-Z0-9_-]{10,})/),b=v.match(/[?&]id=([a-zA-Z0-9_-]{10,})/),c=v.match(/\/d\/([a-zA-Z0-9_-]{10,})/);id=(a||b||c)?.[1]||'';try{const u=new URL(v);resourceKey=u.searchParams.get('resourcekey')||u.searchParams.get('resourceKey')||''}catch{const m=v.match(/[?&]resourcekey=([^&#]+)/i);if(m)try{resourceKey=decodeURIComponent(m[1])}catch{resourceKey=m[1]}}}return {id,resourceKey:String(resourceKey||'').trim()}};
 const driveId=v=>driveRef(v).id;
 const driveStreamCandidates=(id,resourceKey='')=>{const lx=LX.driveStorage?.streamUrl?.(clean(id),clean(resourceKey))||'';return lx?[lx]:[]};
 ...
 function normalize(provider,value){
  ...
  if(provider==='gdrive'){
   const d=driveRef(value),id=d.id;if(!id)fail('GDRIVE_LINK_INVALID','Não reconheci o link do Google Drive. Use o link de compartilhamento do arquivo.');
   return `gdrive:${id}${d.resourceKey?`?resourcekey=${encodeURIComponent(d.resourceKey)}`:''}`;
  }
  if(provider==='dropbox'){
  ...
  if(provider==='b2'){
   const u=https(value);if(!u)fail('B2_LINK_INVALID','Cole uma URL HTTPS de entrega do Backblaze B2/CDN.');
   return `b2:${encodeURIComponent(u.toString())}`;
  }
  if(provider==='direct'){
 ...
 function describe(ref){
  ...
  if(/^https?:\/\//i.test(raw)){
   ...
   const dr=driveRef(raw),did=dr.id;if(did&&/drive\.google\.com/i.test(raw)){const streamCandidates=driveStreamCandidates(did,dr.resourceKey);return {kind:'direct',provider:'LX Storage · Drive',label:'LX Storage · Google Drive',src:streamCandidates[0]||'',streamCandidates,driveId:did,resourceKey:dr.resourceKey}};
  ...
  if(raw.startsWith('gdrive:')){const dr=driveRef(raw),id=dr.id,streamCandidates=driveStreamCandidates(id,dr.resourceKey);return {kind:'direct',provider:'LX Storage · Drive',label:'LX Storage · Google Drive',src:streamCandidates[0]||'',streamCandidates,driveId:id,resourceKey:dr.resourceKey}};
  ...
  if(raw.startsWith('b2:')){const src=decodeURIComponent(raw.slice(3));return {kind:'direct',provider:'LX Cloud · B2',label:'LX Cloud · Backblaze B2',src,openUrl:src,b2:true}};
 ...
 function toInput(ref){const raw=clean(ref);if(raw.startsWith('gdrive:')){const {id,resourceKey}=driveRef(raw);return `https://drive.google.com/file/d/${encodeURIComponent(id)}/view${resourceKey?`?resourcekey=${encodeURIComponent(resourceKey)}`:''}`}if(raw.startsWith('youtube-playlist:'))return `https://www.youtube.com/playlist?list=${raw.slice(17)}`;if(raw.startsWith('youtube:'))return `https://www.youtube.com/watch?v=${raw.slice(8)}`;if(raw.startsWith('spotify:')){const [,type,id]=raw.split(':');return `https://open.spotify.com/${type}/${id}`}if(raw.startsWith('soundcloud:'))return decodeURIComponent(raw.slice(11));if(raw.startsWith('onedrive:'))return decodeURIComponent(raw.slice(9));if(raw.startsWith('archive:'))return `https://archive.org/details/${decodeURIComponent(raw.slice(8))}`;if(raw.startsWith('b2:'))return decodeURIComponent(raw.slice(3));if(raw.startsWith('dropbox:'))return decodeURIComponent(raw.slice(8)).replace(/([?&])raw=1(?:&|$)/,'$1').replace(/[?&]$/,'');return /^https:\/\//i.test(raw)?raw:''}
 ...
 const info={
  ...
  gdrive:{label:'LX Storage · Google Drive',placeholder:'https://drive.google.com/file/d/.../view',help:'Cole o link normal do Drive. A conta de serviço configurada no Worker precisa ter acesso à pasta. Teste o arquivo antes de publicar.'},
  ...
  b2:{label:'Backblaze B2 · LX Cloud',placeholder:'https://f000.backblazeb2.com/file/seu-bucket/filmes/arquivo.mp4 ou https://video.seudominio.com/filmes/master.m3u8',help:'Cole a URL HTTPS de entrega do arquivo no Backblaze B2 ou da sua CDN. MP4 toca direto no LX Player; master.m3u8 ativa qualidade adaptativa. Nunca cole Application Key/Secret no site.'},
 ...
 function allowedFor(type,mode){if(mode==='upload')return true;if(type==='Música')return ['b2','dropbox','direct','youtube','spotify'].includes(mode);if(type==='Livro')return mode==='direct';return ['b2','gdrive','dropbox','youtube','spotify','onedrive','archive','direct'].includes(mode)}
...
function categories(){return state.mode==='Assistir'?['Início','Catálogo Online','Filmes','Séries','Animes','Doramas','Minha Lista','Pedidos','Ranking LX']:state.mode==='Ler'?['Início','Biblioteca Pública','Livros','Minha Lista','Pedidos','Ranking LX']:state.mode==='Ouvir'?['Início','Música','Minha Lista','Pedidos','Ranking LX']:['Transmissões']}
...
function renderHero(){
 const all=items(),f=all.filter(x=>x.featured===true).sort((a,b)=>(+b.priority||0)-(+a.priority||0)),hero=$('hero');hero.classList.toggle('hidden',!f.length);if(!f.length){hero.innerHTML='';return}
 const w=innerWidth,tv=/SmartTV|Tizen|Web0S|webOS|GoogleTV|HbbTV|BRAVIA/i.test(navigator.userAgent)||(!!document.fullscreenElement&&w>=1280),device=tv?'tv':w<=620?'mobile':w<=1100?'tablet':'desktop';state.hero=Math.min(state.hero,f.length-1);
 hero.innerHTML=f.map((x,i)=>{
  const art=x.heroArt?.[device]||x.heroArt?.desktop||x.carouselImage||cinematicArt(x)||x.cover||'';
  const focus=x.heroFocus?.[device]||x.heroFocus?.desktop||{};
  const pos=n=>Math.max(0,Math.min(100,Number(n)||0));
  const zoom=Math.max(1,Math.min(2,Number(focus.zoom)||1)),dim=Math.max(0,Math.min(.85,Number(focus.dim??.45)));
  const image=esc(String(art).replace(/'/g,'%27'));
  return `<article class="hero-slide ${i===state.hero?'active':''}" data-carousel-title="${Number(x.id)}" style="--hero-dim:${dim};--hero-zoom:${zoom};--hero-focus-x:${pos(focus.x??50)}%;--hero-focus-y:${pos(focus.y??35)}%"><div class="hero-bg" data-carousel-configured="true" style="--hero-art:url('${image}');background-image:url('${image}')"></div><div class="hero-copy"><span class="eyebrow">LX PLUS</span>${x.showCarouselTitle===false?'':`<h1>${esc(x.title)}</h1>`}<div class="hero-meta"><span>${esc(x.year||'')}</span><span>${esc(x.rating||'')}</span><span>${esc(x.genre||'')}</span><span>${esc(x.type||'')}</span></div><p>${esc(x.desc||'')}</p><div class="hero-actions"><button class="primary-btn" onclick="LX.primary(${Number(x.id)})">${watchLabel(x)}</button><button class="secondary-btn" onclick="LX.detail(${Number(x.id)})">ⓘ Mais informações</button></div></div></article>`
 }).join('')+`<div class="hero-dots">${f.map((_,i)=>`<button class="${i===state.hero?'active':''}" data-dot="${i}" aria-label="Destaque ${i+1}"></button>`).join('')}</div>`;
 $$('[data-dot]').forEach(d=>d.onclick=()=>{state.hero=+d.dataset.dot;renderHero()});heroTimer(f.length)
}
...
function syncMusicProviderBrand(item=LX.currentMusic?.()){const el=$('musicProviderBrand');if(!el)return;const content=item?.contentId?D.catalog().find(x=>String(x.id)===String(item.contentId))||{}:{},kind=musicProviderKind(item||{},content);el.classList.toggle('hidden',!item);el.innerHTML=item?musicProviderBrandHtml(item,true):'';el.dataset.provider=kind}
LX.syncMusicProviderBrand=syncMusicProviderBrand;LX.musicProviderBrandHtml=musicProviderBrandHtml;
...
LX.musicGenresOf=musicGenresOf;LX.musicHasGenre=musicHasGenre;LX.musicCatalog=musicCatalog;LX.musicCategoryList=musicCategoryList;LX.musicCategoryDefaults=DEFAULT_MUSIC_CATEGORIES;
...
function musicCollections(){const list=S.read(S.keys.musicCollections,[]);return Array.isArray(list)?list:[]}
function musicCollectionsView(all){
 const collections=musicCollections(),selected=collections.find(c=>c.id===state.musicCollectionSelected),byId=new Map(all.map(item=>[String(item.id),item]));
 const header=`<section class="lx-music-collections-head"><div><span class="eyebrow">SUA LX MUSIC</span><h1>${selected?esc(selected.title):'Playlists e álbuns'}</h1><p>${selected?`${selected.kind==='album'?'Álbum pessoal':'Playlist'} · ${(selected.items||[]).length} músicas`:'Crie suas próprias coleções com as músicas do catálogo.'}</p></div><div class="lx-music-collection-actions">${selected?'<button type="button" data-lx-collection-back>← Voltar</button>':''}<button type="button" data-lx-collection-new="playlist">＋ Nova playlist</button><button type="button" data-lx-collection-new="album">＋ Novo álbum</button></div></section>`;
 if(!selected)return header+(collections.length?`<div class="lx-music-collections-grid">${collections.map(c=>{const cover=(c.items||[]).map(id=>byId.get(String(id))).find(Boolean);return `<article class="lx-music-collection-card"><button type="button" data-lx-collection-open="${esc(c.id)}">${cover?LX.artwork.markup(cover,{},'lx-music-collection-cover',cover.title):'<span class="lx-music-collection-cover placeholder">♫</span>'}<strong>${esc(c.title)}</strong><small>${c.kind==='album'?'Álbum pessoal':'Playlist'} · ${(c.items||[]).length} músicas</small></button><button type="button" class="lx-music-collection-play" data-lx-collection-play="${esc(c.id)}" aria-label="Reproduzir ${esc(c.title)}">▶</button></article>`}).join('')}</div>`:musicEmpty('Nenhuma coleção ainda','Crie uma playlist ou álbum e adicione músicas pelo botão ＋ em cada capa.'));
 const rows=(selected.items||[]).map((id,i)=>{const item=byId.get(String(id));return item?`<div class="lx-music-collection-track" data-music-id="${esc(id)}"><button type="button" data-lx-collection-track="${esc(id)}" data-lx-collection-index="${i}">${LX.artwork.markup(item,{},'lx-music-collection-cover',item.title)}<span><strong>${esc(item.title)}</strong><small>${esc(item.artist||'LX Music')}</small></span><i>▶</i></button><button type="button" data-lx-collection-remove="${esc(id)}" aria-label="Remover ${esc(item.title)}">×</button></div>`:`<div class="lx-music-collection-track missing"><span>Música indisponível no catálogo</span><button type="button" data-lx-collection-remove="${esc(id)}" aria-label="Remover música indisponível">×</button></div>`}).join('');
 return `${header}<div class="lx-music-collection-actions"><button type="button" data-lx-collection-play="${esc(selected.id)}">▶ Reproduzir tudo</button><button type="button" data-lx-collection-edit="${esc(selected.id)}">Editar nome e músicas</button><button type="button" data-lx-collection-delete="${esc(selected.id)}">Excluir coleção</button></div>${rows?`<div class="lx-music-collection-tracks">${rows}</div>`:musicEmpty('Coleção vazia','Abra uma música e use ＋ para adicioná-la aqui.')}`
}
function musicAutoAlbums(all){return window.LXAlbumGrouping?.groupAlbums?.(all)||[]}
function musicAutoAlbumsSection(all){const albums=musicAutoAlbums(all);if(!albums.length)return '';return `<section class="lx-music-section lx-music-auto-albums"><div class="lx-section-title"><div><h2>Álbuns pela mesma capa</h2><p>Faixas com capa igual ficam juntas. O nome vem das tags do arquivo ou dos metadados encontrados.</p></div><span>${albums.length} ${albums.length===1?'álbum':'álbuns'}</span></div><div class="lx-music-collections-grid">${albums.map(group=>`<article class="lx-music-collection-card"><button type="button" data-lx-auto-album="${esc(group.id)}">${LX.artwork.markup({cover:group.cover},{},'lx-music-collection-cover',group.title)}<strong>${esc(group.title)}</strong><small>${esc(group.artist)} · ${group.entries.length} faixas</small></button></article>`).join('')}</div></section>`}
function openMusicAutoAlbum(id){const album=musicAutoAlbums(musicCatalog()).find(group=>group.id===id);if(!album)return LX.toast('Este álbum não está disponível.');const first=album.entries[0],modal=$('modal');modal.innerHTML=`<button class="close-btn" onclick="LX.ui.close()">×</button><div class="lx-album-page lx-album-page-v256 lx-album-page-v260"><section class="lx-album-hero"><div class="lx-album-cover" id="lxAutoAlbumCover"></div><div class="lx-album-copy"><span class="eyebrow">LX MUSIC · ÁLBUM AUTOMÁTICO</span><h2>${esc(album.title)}</h2><p>${esc(album.artist)} · ${album.entries.length} faixas</p><div class="hero-actions"><button class="primary-btn" type="button" data-lx-auto-track-id="${esc(first.id)}" data-lx-auto-track-index="${first.index}">▶ Reproduzir álbum</button></div></div></section><div class="lx-track-list lx-track-list-v256"><div class="lx-track-list-head"><span>#</span><span>Título</span><span>Duração</span><span></span></div>${album.entries.map((entry,i)=>`<button class="lx-track-row" type="button" data-lx-auto-track-id="${esc(entry.id)}" data-lx-auto-track-index="${entry.index}"><b>${i+1}</b><span><strong>${esc(entry.title)}</strong><small>${esc(entry.artist||album.artist)}</small></span><em>${LX.fmt(entry.duration)}</em><i>▶</i></button>`).join('')}</div></div>`;const art=modal.querySelector('#lxAutoAlbumCover');if(art)art.style.backgroundImage=`url(${JSON.stringify(album.cover)})`;modal.onclick=e=>{const btn=e.target.closest('[data-lx-auto-track-id]');if(btn){music(btn.dataset.lxAutoTrackId,Number(btn.dataset.lxAutoTrackIndex)||0,true,album.itemIds);U.close()}};$('overlay').classList.remove('hidden')}
LX.openMusicAutoAlbum=openMusicAutoAlbum;
function musicAlbumCard(x){const tracks=x.tracks?.length||1,provider=musicProvider(x),saved=musicSaved(x.id);return `<article class="lx-music-album ${saved?'is-saved':''}" data-music-id="${esc(x.id)}"><button class="lx-music-card-main" type="button" data-lx-play-id="${esc(x.id)}" title="Reproduzir ou pausar ${esc(x.title)}">${LX.artwork.markup(x,{},'lx-music-art',x.title)}<em>${esc(provider)}</em><i data-music-play-icon>${LX.artwork.icon('play')}</i></button><div class="lx-music-card-copy"><button type="button" data-lx-open-album="${esc(x.id)}" title="Abrir álbum de ${esc(x.title)}"><strong>${esc(x.title)}</strong><small>${esc(x.artist||'LX Music')} · ${tracks} ${tracks===1?'faixa':'faixas'}</small></button><button class="lx-music-collection-add" type="button" data-lx-add-collection="${esc(x.id)}" title="Adicionar à playlist ou álbum" aria-label="Adicionar ${esc(x.title)} à playlist ou álbum">${LX.artwork.icon('playlistAdd')}</button><button class="lx-music-save" type="button" data-music-save-id="${esc(x.id)}" onclick="LX.musicToggleSaved(${Number(x.id)})" aria-label="${saved?'Remover da':'Salvar na'} biblioteca" title="${saved?'Remover da':'Salvar na'} biblioteca">${LX.artwork.icon('heart')}</button></div></article>`}
...
function musicNav(active,total,saved,recent){const nav=(view,icon,label,count='')=>`<button type="button" data-lx-music-view="${view}" class="${active===view?'active':''}"><span>${LX.artwork.icon(icon)}</span><b>${label}</b>${count!==''?`<i>${count}</i>`:''}</button>`;return `<aside class="lx-music-side"><div class="lx-music-side-brand"><span>${LX.artwork.icon('play')}</span><div><b>LX Music</b><small>Seu som, do seu jeito</small></div></div><nav>${nav('home','home','Início')}${nav('explore','search','Buscar')}${nav('library','library','Sua biblioteca',saved)}${nav('collections','library','Playlists e álbuns',musicCollections().length)}${nav('liked','heart','Músicas curtidas',saved)}${nav('recent','history','Tocadas recentemente',recent)}</nav><div class="lx-music-side-library"><span>SUA COLEÇÃO</span><button type="button" data-lx-collection-new="playlist"><i>${LX.artwork.icon('playlistAdd')}</i><div><b>Nova playlist</b><small>Organize suas músicas</small></div></button><button type="button" onclick="LX.openMusicQueue()"><i>${LX.artwork.icon('queue')}</i><div><b>Fila de reprodução</b><small>Veja o que toca em seguida</small></div></button></div><div class="lx-music-side-note"><span>PLAYER UNIVERSAL</span><small>${total} ${total===1?'música':'músicas'} no catálogo</small></div></aside>`}
...
function bindMusicExperience(){
 const root=$('homeContent');if(root)root.onclick=e=>{
  const hit=e.target.closest('button');if(!hit||!root.contains(hit))return;
  if(hit.dataset.lxPlayId){LX.music(hit.dataset.lxPlayId,0);return}
  if(hit.dataset.lxOpenAlbum){LX.openMusicAlbum(hit.dataset.lxOpenAlbum);return}
  if(hit.dataset.lxAddCollection){LX.openMusicCollectionPicker(hit.dataset.lxAddCollection);return}
  if(hit.dataset.lxCollectionNew){LX.openMusicCollectionEditor(null,hit.dataset.lxCollectionNew);return}
  if(hit.dataset.lxAutoAlbum){LX.openMusicAutoAlbum(hit.dataset.lxAutoAlbum);return}
  if(hit.dataset.lxCollectionOpen){state.musicCollectionSelected=hit.dataset.lxCollectionOpen;renderApp();return}
  if(hit.hasAttribute('data-lx-collection-back')){state.musicCollectionSelected=null;renderApp();return}
  if(hit.dataset.lxCollectionPlay){LX.playMusicCollection(hit.dataset.lxCollectionPlay);return}
  if(hit.dataset.lxCollectionTrack){LX.playMusicCollection(state.musicCollectionSelected,hit.dataset.lxCollectionTrack);return}
  if(hit.dataset.lxCollectionRemove){LX.removeMusicCollectionTrack(state.musicCollectionSelected,hit.dataset.lxCollectionRemove);return}
  if(hit.dataset.lxCollectionEdit){LX.openMusicCollectionEditor(hit.dataset.lxCollectionEdit);return}
  if(hit.dataset.lxCollectionDelete)LX.deleteMusicCollection(hit.dataset.lxCollectionDelete);
 };
...
function renderMusicExperience(){
 ...
 const side=musicNav(active,all.length,saved.length,recent.length),mobile=`<div class="lx-music-mobile-tabs"><button type="button" data-lx-music-view="home" class="${active==='home'?'active':''}">Início</button><button type="button" data-lx-music-view="explore" class="${active==='explore'?'active':''}">Buscar</button><button type="button" data-lx-music-view="library" class="${['library','liked','recent'].includes(active)?'active':''}">Biblioteca</button><button type="button" data-lx-music-view="collections" class="${active==='collections'?'active':''}">Playlists</button></div>`;
 if(active==='collections'){$('homeContent').innerHTML=`<div class="lx-music-app-shell lx-music-shell-v260">${side}<main class="lx-music-main lx-music-main-v260">${mobile}${musicCollectionsView(all)}</main>${musicDetailRail(all)}</div>`;bindMusicExperience();LX.artwork.hydrate?.($('homeContent'));syncMusicDetailRail();return}
 ...
 if(active==='home'){
  ...
  const subtitle=active==='liked'?'Tudo o que você salvou em um só lugar.':active==='recent'?'Continue ouvindo de onde parou.':active==='library'?'Seus salvos e reproduções recentes.':categoryTitle?`Todas as músicas encontradas na categoria ${categoryTitle}.`:'Pesquise por música, artista, álbum ou gênero. Toque na capa ou no título para ouvir.';
...
(()=>{
  ...
  async function importYoutubeBulk(){
    ...
    try{
      const out=await youtubeInvoke(source),rows=out.items||[];if(btn)btn.textContent='Buscando capas e metadados…';setYoutubeBulkStatus(`<b>Identificando ${rows.length} músicas…</b><span>Conferindo dados do próprio vídeo e metadados complementares antes de criar rascunhos.</span>`,'loading');const metaMap=await youtubeMetadataBatch(rows),existing=D().catalog().filter(x=>x.type==='Música'),ids=new Set(existing.map(youtubeIdOfCatalog).filter(Boolean)),fps=new Set(existing.map(x=>musicFingerprint(x.title,x.artist)).filter(x=>x!=='|')),batchIds=new Set(),batchFps=new Set(),fresh=[];let duplicates=0,enriched=0;
      ...
      setYoutubeBulkStatus(`<b>${saved} ${saved===1?'música em rascunho salva':'músicas em rascunho salvas'}.</b><span>${found} encontradas · ${enriched} com metadados/capa enriquecidos · ${duplicates} repetidas ignoradas · categoria: ${esc(category)}.</span>${truncated}`,'ok');
      toast(saved?`${saved} músicas importadas para revisão no ADM → Músicas. ${duplicates} repetidas foram ignoradas.`:`Nenhuma música nova. ${duplicates||found} já estavam no catálogo.`);
  ...
  function render(m){
    ...
    LX.recovery?.renderSpotifyBulk?.(m);
  }
...
(()=>{
 ...
 LX.contentHub={invoke,renderWatch,openWatch,importWatch,renderBooks,importBook,importBookAt:i=>importBook(cache.books[+i]),renderMusic,playMusicAt:i=>LX.playOnlineMusicPreview(cache.music[+i])};
})();
...
function adminMusicHasMp3(item){
 const refs=(item.tracks?.length?item.tracks:[item]).map(t=>t?.mediaKey||t?.authorizedAudioUrl||t?.url||item.mediaKey||item.externalMusicUrl||'');
 return !!refs.length&&refs.every((ref,i)=>/\.mp3(?:$|[?#])/i.test(ref)||((item.tracks?.[i]?.mimeType||item.mimeType)==='audio/mpeg'));
}
function catalogSection(m,page){
 ...
 m.innerHTML=head(config.title,`Gerencie ${config.title.toLowerCase()} em uma área própria. Alterações e exclusões são confirmadas pelo catálogo da nuvem.`)+pendingBar(page)+`<div class="stats lx-admin-catalog-stats"><div class="stat"><small>TOTAL</small><strong>${all.length}</strong></div><div class="stat"><small>PUBLICADOS</small><strong>${published}</strong></div><div class="stat"><small>RASCUNHOS</small><strong>${all.length-published}</strong></div><div class="stat"><small>SEM FONTE</small><strong>${needsMedia}</strong></div></div>${page==='music'?musicCategoryAdmin():''}<section class="admin-card lx-admin-catalog-page">${page==='music'?`<div class="lx-admin-source-tabs" role="tablist" aria-label="Tipo de fonte musical"><button type="button" data-source-filter="all" class="active">Todas (${all.length})</button><button type="button" data-source-filter="mp3">MP3 (${all.filter(adminMusicHasMp3).length})</button><button type="button" data-source-filter="links">Links / converter para arquivo próprio (${all.filter(x=>!adminMusicHasMp3(x)).length})</button></div>`:''}<div class="admin-toolbar"><input id="catalogSearch" type="search" placeholder="Buscar ${config.title.toLowerCase()}" aria-label="Buscar ${config.title.toLowerCase()}"><select id="catalogStatus" aria-label="Filtrar por status"><option value="all">Todos os status</option><option value="published">Publicado</option><option value="draft">Rascunho</option><option value="scheduled">Agendado</option></select>${page==='series'?'<select id="catalogSubtype" aria-label="Tipo de série"><option value="all">Séries, animes e doramas</option><option value="Série">Séries</option><option value="Anime">Animes</option><option value="Dorama">Doramas</option></select>':''}<button id="catalogNew" class="primary-btn" type="button">＋ Adicionar ${config.type.toLowerCase()}</button></div><div class="lx-admin-catalog-overflow"><table class="admin-table lx-admin-catalog-table"><thead><tr><th>${page==='music'?'Faixa / artista':'Título'}</th><th>${page==='music'?'Álbum · fonte':page==='series'?'Episódios · fonte':'Dados · fonte'}</th><th>Status</th><th>Ações</th></tr></thead><tbody id="catalogRows"></tbody></table></div><small id="catalogResultCount" class="lx-admin-result-count"></small></section>`;
 const draw=()=>{const q=D.normalize($('catalogSearch')?.value||''),status=$('catalogStatus')?.value||'all',subtype=$('catalogSubtype')?.value||'all',source=m.querySelector('.lx-admin-source-tabs button.active')?.dataset.sourceFilter||'all',now=Date.now(),items=all.filter(x=>{if(subtype!=='all'&&x.type!==subtype)return false;if(source==='mp3'&&!adminMusicHasMp3(x)||source==='links'&&adminMusicHasMp3(x))return false;if(q&&!D.normalize([x.title,x.genre,x.artist,x.author,x.album,x.year,x.type].join(' ')).includes(q))return false;const scheduled=x.published!==false&&x.scheduledAt&&+new Date(x.scheduledAt)>now;return status==='all'||(status==='published'&&x.published!==false&&!scheduled)||(status==='draft'&&x.published===false)||(status==='scheduled'&&scheduled)});$('catalogRows').innerHTML=catalogSectionRows(items,page)||'<tr><td colspan="4" class="lx-admin-empty">Nenhum conteúdo encontrado nesta área.</td></tr>';$('catalogResultCount').textContent=`${items.length} de ${all.length} ${config.title.toLowerCase()}`};
 m.querySelectorAll('.lx-admin-source-tabs button').forEach(b=>b.onclick=()=>{m.querySelectorAll('.lx-admin-source-tabs button').forEach(x=>x.classList.toggle('active',x===b));draw()});
 if(page==='music')LX.recovery?.renderMp3Import?.(m);
...
function uploads(m){
 ...
 <section class="admin-card r2-storage-card lx-drive-storage-card">
  ...
  <div class="form-grid r2-config-grid">
   <div class="span2 lx-storage-auto-box"><div><b>Conexão única</b><small>Implante o Worker, configure a conta de serviço em Secrets da Cloudflare uma vez e dê acesso à pasta do Drive. Depois, basta colar o link do filme aqui no ADM.</small></div><button id="autoDeployDriveStorage" class="primary-btn" type="button">Abrir implantação do Worker</button></div>
   ...
   <div class="span2 r2-config-actions"><button id="saveDriveStorage" class="primary-btn" type="button">Salvar endpoint</button><button id="testDriveStorage" class="glass-btn" type="button">Testar conexão</button><button id="reconnectDriveStorage" class="glass-btn" type="button">Reconectar</button><button id="openDriveFiles" class="glass-btn" type="button">Abrir arquivos ↗</button></div>
   <small class="span2">Espaço usado / disponível: indisponível para o Drive pessoal via conta de serviço.</small>
   <label class="field span2">Testar arquivo antes de publicar<input id="driveStorageFileLink" inputmode="url" placeholder="Cole o link normal do arquivo no Google Drive"></label>
   <div class="span2 r2-config-actions"><button id="probeDriveStorage" class="glass-btn" type="button">Verificar vídeo e Range/206</button><button id="copyDriveDiagnostics" class="glass-btn" type="button">Copiar diagnóstico</button></div>
   <div id="driveStorageProbeResult" class="r2-help span2" role="status">O teste confirma arquivo, acesso e Range. O codec será indicado como desconhecido até a mídia ser analisada ou testada no navegador.</div>
  </div>
  <div id="driveStorageHelp" class="r2-help"><b>Fluxo:</b> Drive API autenticada → LX Storage Worker → LX Player. Espaço usado e disponível não pode ser inferido da conta de serviço para seu Drive pessoal.</div>
 </section>
 ...
 <section class="admin-card free-media-hub-card">
  ...
  <div class="free-media-provider-grid">
   <article><b>Google Drive · LX Storage</b><small>Drive API + Worker → LX Player próprio</small></article><article><b>Diagnóstico</b><small>Teste o arquivo no ADM antes de publicar</small></article>
...
function bind(page){
 ...
 if(page==='uploads'){
  const refreshDriveStorage=async()=>{const badge=$('driveStorageStatusBadge'),help=$('driveStorageHelp'),inp=$('driveStorageEndpoint');if(inp&&!inp.value)inp.value=LX.driveStorage?.endpoint?.()||'';if(!badge)return;const ep=(inp?.value||LX.driveStorage?.endpoint?.()||'').trim();if(!ep){badge.textContent='Endpoint não configurado';badge.className='r2-status warn';if(help)help.textContent='Implante o Worker R12, configure os Secrets na Cloudflare e salve a URL correta.';return}badge.textContent='Testando…';badge.className='r2-status warn';try{const st=await LX.driveStorage.test(ep,true);badge.textContent=st.googleConnected?'● Online · Google Drive conectado':st.googleConfigured?'● Worker R12 online · Google desconectado':'● Worker R12 online · Google não configurado';badge.className='r2-status '+(st.googleConnected?'ok':'warn');if(help)help.textContent=st.googleConnected?'Drive autenticado no Worker R12. Agora teste um arquivo real antes de publicar.':'Conclua a conexão do Google: '+LX.driveStorage.explain(st.error||'DRIVE_AUTH_NOT_CONFIGURED')}catch(e){badge.textContent=e.message==='LX_STORAGE_WORKER_OUTDATED'?'● Worker desatualizado':'● LX Storage indisponível';badge.className='r2-status warn';if(help)help.textContent=LX.driveStorage.explain(e?.message)}};
  ...
  $('saveDriveStorage')?.addEventListener('click',async()=>{const b=$('saveDriveStorage'),ep=$('driveStorageEndpoint')?.value?.trim()||'';b.disabled=true;try{if(ep)await LX.driveStorage.test(ep,true);await LX.driveStorage.saveEndpoint(ep,{global:true});LX.toast(ep?'Endpoint do Worker R12 salvo para todos os dispositivos.':'Endpoint LX Storage removido.');await refreshDriveStorage()}catch(e){console.warn('LX Storage save',e);LX.toast(LX.driveStorage.explain(e?.message))}finally{b.disabled=false}});
  $('testDriveStorage')?.addEventListener('click',refreshDriveStorage);
  $('reconnectDriveStorage')?.addEventListener('click',()=>{const help=$('driveStorageHelp');if(help)help.textContent='Para reconectar, atualize GOOGLE_CLIENT_EMAIL e GOOGLE_PRIVATE_KEY nos Secrets do Worker na Cloudflare, compartilhe a pasta com a conta de serviço e toque em Testar conexão.';LX.toast('As credenciais ficam protegidas na Cloudflare, fora do navegador.')});
  $('openDriveFiles')?.addEventListener('click',()=>window.open('https://drive.google.com/drive/u/0/my-drive','_blank','noopener,noreferrer'));
  let lastDriveDiagnostic=null;
  $('probeDriveStorage')?.addEventListener('click',async()=>{const out=$('driveStorageProbeResult'),b=$('probeDriveStorage'),link=$('driveStorageFileLink')?.value?.trim()||'';if(!out)return;b.disabled=true;out.textContent='Testando arquivo, acesso e Range…';try{const result=await LX.driveStorage.probe(link);lastDriveDiagnostic={time:new Date().toISOString(),endpoint:LX.driveStorage.endpoint(),version:'R12',result};const f=result.file||{};out.textContent='✓ Arquivo encontrado: '+(f.name||'sem nome')+' · '+(Number(f.size||0)/1048576).toFixed(1)+' MB | ✓ Vídeo acessível | '+(result.rangeOk?'✓ Range 206':'✗ Range indisponível')+' | '+(['container_incompatible','codec_incompatible'].includes(result.browserCompatibility)?'✗ Formato/codec incompatível: use MP4 H.264 + AAC':result.codec&&result.codec!=='unknown'?'✓ Codec detectado: '+result.codec:'? Codec ainda não verificado; faça um teste no navegador');}catch(e){lastDriveDiagnostic={time:new Date().toISOString(),endpoint:LX.driveStorage.endpoint(),error:e.message};out.textContent='✗ '+LX.driveStorage.explain(e.message)}finally{b.disabled=false}});
  $('copyDriveDiagnostics')?.addEventListener('click',async()=>{if(!lastDriveDiagnostic)return LX.toast('Teste um arquivo primeiro.');try{await navigator.clipboard.writeText(JSON.stringify(lastDriveDiagnostic,null,2));LX.toast('Diagnóstico copiado.')}catch{LX.toast('Não foi possível copiar o diagnóstico.')}});
  refreshDriveStorage();
 ...
 if(page==='notifications'&&$('sendNotice')){let pendingNotice=null;const preview=()=>{const box=$('noticePreview');if(!box)return;box.querySelector('strong').textContent=$('noticeTitle').value.trim()||'Prévia da notificação';box.querySelector('p').textContent=$('noticeText').value.trim()||'Escreva o título e a mensagem para ver o resultado.'};$('noticeTitle').oninput=preview;$('noticeText').oninput=preview;$('sendNotice').onclick=async()=>{const t=$('noticeTitle').value.trim(),text=$('noticeText').value.trim(),btn=$('sendNotice');if(!t||!text)return LX.toast('Preencha título e mensagem.');btn.disabled=true;btn.textContent='Publicando…';const row=pendingNotice||{id:Date.now(),title:t,text,time:$('noticeTime').value||'Agora',read:false};if(!pendingNotice){const n=D.notices();n.unshift(row);S.writeLocal(S.keys.notices,n)}pendingNotice=row;try{if(!LX.cloud?.enabled?.())throw new Error('SUPABASE_UNAVAILABLE');await LX.cloud.publishNotices([row]);pendingNotice=null;LX.toast('Notificação publicada e confirmada na nuvem.');render('notifications')}catch(error){console.warn('LX notice publish',error);LX.toast('Aviso salvo neste navegador, mas não foi publicado na nuvem. Verifique a conexão e tente novamente.');btn.disabled=false;btn.textContent='Tentar publicar novamente'}}}
...
const DRIVE_QUALITY_KEYS=['2160p','1440p','1080p','720p','480p','360p','240p'];
...
async function edit(id=null,initialType=null,prefill=null){
 ...
 <form id="contentForm" class="quick-content-form">
  <div class="quick-core">
   ...
   <div id="quickDriveReady" class="quick-drive-ready span2 ${['Filme','Série','Anime','Dorama'].includes(type)?'':'hidden'}"><div><span class="quick-drive-dot"></span><strong>LX Storage · Google Drive</strong><small>Google Drive → LX Storage Worker → LX Player. Teste o link do vídeo antes de publicar.</small></div><span id="quickDriveState" class="quick-drive-state">${LX.driveStorage?.configured?.()?'ENDPOINT SALVO':'CONFIGURAR'}</span></div>
   ...
   <div class="media-source-switch media-source-switch-v2542 span2"><button type="button" class="${['Filme','Série','Anime','Dorama'].includes(type)?'active':''}" data-media-source="gdrive">☁ LX Storage / Drive</button><button type="button" class="${['Filme','Série','Anime','Dorama'].includes(type)?'':'active'}" data-media-source="upload">↑ Arquivo/R2</button><button type="button" data-media-source="b2" class="lx-legacy-source">B2 legado</button><button type="button" data-media-source="dropbox">Dropbox</button><button type="button" data-media-source="youtube">YouTube</button><button type="button" data-media-source="spotify">Spotify</button><button type="button" data-media-source="onedrive">OneDrive</button><button type="button" data-media-source="archive">Archive</button><button type="button" data-media-source="direct">HTTPS</button><small>Filmes e séries usam LX Storage / Drive por padrão. Outras fontes continuam disponíveis quando necessário.</small></div>
   ...
   <section id="driveQualityField" class="drive-quality-field span2 hidden">
    ...
    <div class="drive-quality-note"><b>Como funciona:</b> o link principal reproduz o arquivo original pelo LX Storage. Para oferecer outras qualidades, indique arquivos separados nos campos acima; cada escolha troca o arquivo no LX Player.</div>
   </section>
  </div>
 ...
 const updateExternalFields=()=>{const episodic=isEpisodic(),external=mediaSource!=='upload',drive=mediaSource==='gdrive',video=['Filme','Série','Anime','Dorama'].includes($('cType')?.value);$('externalMediaField')?.classList.toggle('hidden',!external||episodic);$('episodeBulkField')?.classList.toggle('hidden',!external||!episodic);$('driveQualityField')?.classList.toggle('hidden',!drive);$('driveMovieQualities')?.classList.toggle('hidden',!drive||episodic);$('driveEpisodeQualities')?.classList.toggle('hidden',!drive||!episodic);$('quickDriveReady')?.classList.toggle('hidden',!video);const qds=$('quickDriveState');if(qds){qds.textContent=LX.driveStorage?.configured?.()?'ENDPOINT SALVO':'CONFIGURAR';qds.classList.toggle('warn',!LX.driveStorage?.configured?.())}if(external&&episodic)fillSeasonLinks();if(drive&&!episodic)fillDriveMovieQualities()};
 ...
 $('testExternalMedia').onclick=async()=>{const target=$('externalMediaDetected'),button=$('testExternalMedia');button.disabled=true;target.textContent='Verificando arquivo…';try{const raw=$('cExternalMedia').value,d=LX.mediaSources.preview(mediaSource,raw);if(mediaSource==='gdrive'){const check=await LX.driveStorage.probe(raw);target.textContent='✓ Arquivo: '+(check.file?.name||'encontrado')+' · '+(check.rangeOk?'Range 206 funcionando':'Range indisponível')+' · '+(['container_incompatible','codec_incompatible'].includes(check.browserCompatibility)?'Formato incompatível. Use MP4 H.264 + AAC.':check.codec&&check.codec!=='unknown'?'Codec detectado: '+check.codec:'Codec ainda não verificado no navegador.');if(!String($('cTitle').value||'').trim()&&check.file?.name)$('cTitle').value=check.file.name.replace(/\.[^.]+$/,'').replace(/[._]+/g,' ').trim();if(!String($('cDesc').value||'').trim()&&check.file?.name)$('cDesc').value='Vídeo: '+check.file.name;if(check.file?.durationMillis&&!x)$('cDuration')&&($('cDuration').value=Math.round(Number(check.file.durationMillis)/60000));}else{target.textContent=`✓ ${d.provider} reconhecido · pronto para salvar`;if($('cType').value==='Música'&&mediaSource==='youtube')await LX.musicMeta?.fillEditorFromYoutube?.(raw,{force:!x});else if(d.openUrl)window.open(d.openUrl,'_blank','noopener')}}catch(err){target.textContent='✗ '+(mediaSource==='gdrive'?LX.driveStorage.explain(err.message):(err.message||'Não reconheci esse link.'))}finally{button.disabled=false}};
 $('cExternalMedia')?.addEventListener('input',()=>{const raw=$('cExternalMedia')?.value?.trim()||'';if(/drive\.google\.com\/(?:file\/d\/|open\?|uc\?)/i.test(raw)&&$('cType')?.value!=='Música'&&mediaSource!=='gdrive')setMediaSource('gdrive');updateMusicIdentity()});$('cExternalMedia')?.addEventListener('blur',()=>{if($('cType')?.value==='Música'&&mediaSource==='youtube'&&$('cExternalMedia')?.value?.trim())LX.musicMeta?.fillEditorFromYoutube?.($('cExternalMedia').value.trim(),{force:!x}).catch(()=>{})});
 ...
 $('contentForm').onsubmit=async e=>{
  ...
  try{
   ...
   if(mediaSource==='gdrive'&&y.published){
    const links=[];if(['Série','Anime','Dorama'].includes(y.type)){links.push(...parseEpisodeLinks($('cEpisodeLinks')?.value||'','gdrive').map(r=>r.mediaKey));for(const q of DRIVE_QUALITY_KEYS){const v=$(driveQualityInputId(q,true))?.value||'';if(v.trim())links.push(...parseEpisodeLinks(v,'gdrive').map(r=>r.mediaKey))}}
    else{const main=$('cExternalMedia')?.value?.trim();if(main)links.push(main);for(const q of DRIVE_QUALITY_KEYS){const v=$(driveQualityInputId(q))?.value?.trim();if(v)links.push(v)}}
    const unique=[...new Set(links)];for(let i=0;i<unique.length;i+=4){status.textContent='Verificando vídeos no Drive '+Math.min(i+4,unique.length)+'/'+unique.length+'…';const reports=await Promise.all(unique.slice(i,i+4).map(link=>LX.driveStorage.probe(link)));for(const check of reports){if(!check.rangeOk)throw new Error('DRIVE_RANGE_UNAVAILABLE');if(['container_incompatible','codec_incompatible'].includes(check.browserCompatibility))throw new Error('DRIVE_FORMAT_INCOMPATIBLE')}}
   }
   ...
   else if(/B2_LINK_INVALID/.test(code))msg='Backblaze B2: cole uma URL HTTPS pública/CDN do arquivo ou playlist HLS. Não cole chaves secretas.';
   else if(/GDRIVE_LINK_INVALID/.test(code))msg='Google Drive: copie o link do arquivo. A conta de serviço deve ter acesso à pasta.';
   else if(/DRIVE_AUTH_NOT_CONFIGURED|DRIVE_AUTH_INVALID|DRIVE_PERMISSION_OR_QUOTA|FILE_NOT_FOUND_OR_NO_ACCESS|DRIVE_RANGE_UNAVAILABLE|DRIVE_RETURNED_NON_MEDIA|NOT_A_MEDIA_FILE|DRIVE_FORMAT_INCOMPATIBLE/.test(code))msg=LX.driveStorage.explain(code);
...
// R5 bridge: helpers implemented by ui.js are exported on LX because app.js runs in a separate IIFE.
const musicGenresOf=x=>LX.musicGenresOf?.(x)||[String(x?.genre||'Outros')];
const musicHasGenre=(x,genre)=>LX.musicHasGenre?.(x,genre)??(genre==='Todos'||musicGenresOf(x).some(g=>D.normalize(g)===D.normalize(genre)));
const musicCatalog=()=>LX.musicCatalog?.()||D.catalog().filter(x=>x.published!==false&&x.type==='Música');
...
function shellModeCategory(mode){return mode==='Assistir'?'Início':mode==='Ouvir'?'Início':mode==='Ler'?'Início':mode==='Ao vivo'?'Transmissões':'Início'}
...
$$('[data-mode]').forEach(b=>b.onclick=()=>{state.mode=b.dataset.mode;state.category=b.dataset.mode==='Ao vivo'?'Transmissões':'Início';state.query='';if($('searchInput'))$('searchInput').value='';$$('[data-mode]').forEach(x=>x.classList.toggle('active',x===b));U.renderApp();syncMobileNavState();saveView()});
...
async function play(id,epNum=1,epSeason=null){
 ...
 const epIndex=ep?orderedEpisodes.findIndex(e=>e===ep):-1,nextEp=epIndex>=0?orderedEpisodes[epIndex+1]:null,prevEp=epIndex>0?orderedEpisodes[epIndex-1]:null;
 ...
 const resolveMedia=ref=>{if(!ref)return null;const info=LX.mediaSources?.describe?.(ref);if(info?.kind==='direct'){const rows=Array.isArray(info.streamCandidates)?info.streamCandidates.filter(Boolean):[];if(rows.length){const idx=Math.max(0,Math.min(rows.length-1,Number(mediaCandidateIndex.get(String(ref))||0)));return rows[idx]}return info.src}if(/^https?:\/\//i.test(String(ref)))return ref;return Promise.resolve(S.getMedia(ref)).then(b=>b?(typeof b==='string'?b:URL.createObjectURL(b)):null)};
 ...
 // Keep the picture clear while it plays; show a compact cinema bar on interaction.
 const cinemaStyle=document.createElement('style');cinemaStyle.textContent=`
 .root.playing .center{opacity:0!important;pointer-events:none!important;transform:translate(-50%,-50%) scale(.94)}
 .root.paused .center{opacity:1!important;pointer-events:auto!important}
 .root.playing.idle .top,.root.playing.idle .bottom,.root.playing.idle .shadeTop,.root.playing.idle .shadeBottom{opacity:0!important;pointer-events:none!important}
 .root.paused .shadeTop,.root.paused .shadeBottom{opacity:1}
 .top{padding-top:max(22px,env(safe-area-inset-top));gap:16px}
 .heading strong{font-size:clamp(19px,1.6vw,27px);font-weight:750;letter-spacing:-.025em}
 .heading small{font-size:12px;color:rgba(255,255,255,.74)}
 .origin{font-size:9px;color:rgba(255,255,255,.8);border-color:rgba(255,255,255,.13)}
 .bottom{padding-top:36px;background:linear-gradient(0deg,rgba(0,0,0,.66),transparent)}
 .timeline{padding:12px 0 16px}.progress{height:4px;transition:height .16s ease}
 .timeline:hover .progress,.progress:focus-visible{height:7px}
 .round,.tool,.seek{transition:background .16s ease,transform .16s ease,border-color .16s ease}
 .round:hover,.tool:hover,.seek:hover{border-color:rgba(255,255,255,.58)}
 .mainPlay{width:76px;height:76px;font-size:32px;box-shadow:0 16px 55px rgba(0,0,0,.48)}
 .seek{width:50px;height:50px}.lx-quick-seek{font-size:12px;font-weight:750;letter-spacing:-.04em}
 button:focus-visible,input:focus-visible{outline:2px solid #fff;outline-offset:3px}
 @media(max-width:760px){.top{padding-top:max(12px,env(safe-area-inset-top));gap:8px}.heading strong{font-size:16px}.heading small{font-size:10px}.bottom{padding-top:16px}.mainPlay{width:68px;height:68px}.seek{width:46px;height:46px}}
 @media(max-width:460px){.lx-quick-seek{display:none}.heading strong{font-size:14px}}
 ...
 const setUI=()=>{const paused=v.paused;root.classList.toggle('paused',paused);root.classList.toggle('playing',!paused);playBtn.textContent=paused?'▶':'❚❚';playSmall.textContent=paused?'▶':'❚❚';playBtn.setAttribute('aria-label',paused?'Reproduzir':'Pausar');playSmall.setAttribute('aria-label',paused?'Reproduzir':'Pausar');if(!paused)posterEl?.classList.add('hide')};
 const scheduleHideControls=()=>{clearTimeout(hideTimer);hideTimer=null;if(!v.paused&&!nativeMode&&settings.classList.contains('hide'))hideTimer=setTimeout(()=>{hideTimer=null;root.classList.add('idle')},3000)};
 const showControls=()=>{root.classList.remove('idle');scheduleHideControls()};
 ...
 for(const [seconds,label] of [[-10,'Voltar 10 segundos'],[10,'Avançar 10 segundos']]){const button=document.createElement('button');button.type='button';button.className='round lx-quick-seek';button.textContent=seconds<0?'↶10':'↷10';button.setAttribute('aria-label',label);button.onclick=()=>seek(seconds);q('.left').insertBefore(button,q('#mute'))}
 ...
 const activateDriveFallback=()=>false;
 const scheduleDriveFallback=()=>{};
  const setSource=async(keepTime=0,autoplay=false)=>{deactivateDriveFallback();loading.classList.remove('hide');error.classList.add('hide');try{const info=LX.mediaSources?.describe?.(key)||sourceInfo;if(info.driveId)await LX.driveStorage.ensureReady(sourceRefreshes>0);const resolved=resolveMedia(key),next=typeof resolved==='string'?resolved:await resolved;if(!next)throw new Error(info.driveId?'LX_STORAGE_ENDPOINT_MISSING':'MEDIA_NOT_AVAILABLE');if(currentObjectUrl?.startsWith('blob:'))try{URL.revokeObjectURL(currentObjectUrl)}catch{}currentObjectUrl=next.startsWith?.('blob:')?next:null;pendingSeek=keepTime>0?keepTime:null;if(/\.(?:m3u8|mpd)(?:$|[?#])/i.test(next)&&LX.adaptive?.load)await LX.adaptive.load(v,next);else{if(sourceRefreshes)await LX.adaptive?.destroy?.(v);v.src=next;v.load()}sourceRefreshes++;scheduleDriveFallback();if(autoplay){v.play().catch(err=>{if(err?.name==='NotAllowedError'){loading.classList.add('hide');errorText.textContent='Toque em ▶ para iniciar o filme neste navegador.';error.classList.remove('hide');showControls()}else console.warn('LX initial video playback',err)})}return true}catch(err){console.warn('LX player source',err);loading.classList.add('hide');const info=LX.mediaSources?.describe?.(key)||sourceInfo;errorText.textContent=info.driveId?LX.driveStorage.explain(err?.message):'A fonte do vídeo não pôde ser carregada. Verifique a conexão e tente novamente.';error.classList.remove('hide');return false}};
 const startTV=async()=>{
  try{
   if(typeof v.webkitShowPlaybackTargetPicker==='function'){v.webkitShowPlaybackTargetPicker();return}
   let src=v.currentSrc||v.src||'';
   if(!/^https:\/\//i.test(src)&&String(key).startsWith('cloud:'))src=await LX.cloud.mediaUrl(String(key).slice(6),43200);
   if(!/^https:\/\//i.test(src)&&String(key).startsWith('r2:'))src=await LX.r2?.mediaUrl?.(key);
   if(LX.cast?.init?.()&&/^https:\/\//i.test(src)&&!root.classList.contains('drive-fallback')){
    const contentType=/\.m3u8(?:$|[?#])/i.test(src)?'application/x-mpegURL':/\.mpd(?:$|[?#])/i.test(src)?'application/dash+xml':'video/mp4';
    await LX.cast.send({url:src,contentType,title:x.title,subtitle:ep?.title||'',image:x.banner||x.cover||'',currentTime:v.currentTime||0});
    v.pause();LX.toast('Reprodução enviada para a TV.');return
   }
   if(v.remote&&typeof v.remote.prompt==='function'&&!root.classList.contains('drive-fallback')){await v.remote.prompt();return}
   LX.toast('Transmissão disponível para arquivos MP4/HLS/MPD diretos em TVs compatíveis. Abra LX Plus no navegador da TV para outras fontes.')
  }catch(err){if(err?.name!=='AbortError'){console.warn('LX TV',err);LX.toast('TV não conectada. Confira se está na mesma rede e se o arquivo é compatível.')}}
 };
 ...
 const toggleNative=()=>{nativeMode=!nativeMode;v.controls=nativeMode;root.classList.toggle('native',nativeMode);settings.classList.add('hide');showControls();LX.toast(nativeMode?'Modo simples ativado.':'Controles LX ativados.')};
 ...
 v.controls=false;v.playsInline=true;v.disableRemotePlayback=false;v.volume=Math.max(0,Math.min(1,Number(pref.volume??1)));v.muted=!!pref.muted;
 ...
 v.onerror=async()=>{loading.classList.add('hide');const keep=Number.isFinite(v.currentTime)?v.currentTime:0,currentInfo=LX.mediaSources?.describe?.(key)||sourceInfo;if(currentInfo.driveId){errorText.textContent='Diagnosticando arquivo no LX Storage…';error.classList.remove('hide');try{const data=await LX.driveStorage.probe(key);errorText.textContent=['container_incompatible','codec_incompatible'].includes(data.browserCompatibility)?LX.driveStorage.explain('DRIVE_FORMAT_INCOMPATIBLE'):data.rangeOk?'O vídeo chegou pelo Drive, mas o navegador não conseguiu decodificá-lo. Confira os codecs; recomendamos MP4 H.264 + AAC.':'O Drive não entregou Range/206 para este arquivo.'}catch(e){errorText.textContent=LX.driveStorage.explain(e?.message)}return}if(sourceRefreshes<2){errorText.textContent='Renovando a fonte segura do vídeo…';error.classList.remove('hide');await setSource(keep);return}errorText.textContent='O arquivo não pôde ser reproduzido neste navegador. Tente outra fonte compatível com MP4/HLS.';error.classList.remove('hide');v.controls=true;nativeMode=true;root.classList.add('native')};
 v.onplay=()=>{setUI();showControls()};v.onpause=()=>{clearTimeout(hideTimer);setUI();root.classList.remove('idle');if(v.duration)saveProgress(id,v.currentTime/v.duration*100,v.currentTime,v.duration,context,ep)};
 ...
 v.onended=()=>{clearTimeout(hideTimer);saveProgress(id,100,v.duration,v.duration,context,ep);setUI();root.classList.remove('idle');q('#nextCard')?.classList.remove('hide');if(nextEp){let left=5;const card=q('#nextCard'),btn=q('#nextBtn');if(card){const s=card.querySelector('strong'),playNext=()=>{clearInterval(timer);stopMiniPlayer(true);setTimeout(()=>play(id,nextEp.number,nextEp.season||1),40)},cancel=document.createElement('button');cancel.id='nextCancel';cancel.textContent='Cancelar próximo episódio';cancel.style.cssText='margin-left:7px;background:rgba(255,255,255,.1);color:#fff';btn?.insertAdjacentElement('afterend',cancel);if(s)s.textContent=`Próximo episódio em ${left}s`;const timer=setInterval(()=>{if(!document.getElementById('lxGlobalCinema'))return clearInterval(timer);left--;if(left<=0)playNext();else if(s)s.textContent=`Próximo episódio em ${left}s`},1000);if(btn)btn.onclick=playNext;cancel.onclick=()=>{clearInterval(timer);card.classList.add('hide');if(s)s.textContent='Próximo episódio cancelado'}}}};
 playBtn.onclick=playSmall.onclick=togglePlay;q('#prevNow')?.addEventListener('click',()=>{if(prevEp){stopMiniPlayer(true);setTimeout(()=>play(id,prevEp.number,prevEp.season||1),40)}});q('#nextNow')?.addEventListener('click',()=>{if(nextEp){stopMiniPlayer(true);setTimeout(()=>play(id,nextEp.number,nextEp.season||1),40)}});q('#rew').onclick=()=>seek(-10);q('#fwd').onclick=()=>seek(10);progress.oninput=()=>{if(v.duration)v.currentTime=(+progress.value/100)*v.duration};q('#mute').onclick=()=>{v.muted=!v.muted;q('#mute').textContent=v.muted?'🔇':'🔊';savePref();showControls()};volume.oninput=()=>{v.volume=+volume.value;v.muted=false;q('#mute').textContent='🔊';savePref();showControls()};q('#back').onclick=()=>stopMiniPlayer(true);q('#cast').onclick=startTV;q('#full').onclick=fullscreen;q('#pip').onclick=async()=>{if(v.requestPictureInPicture)try{await v.requestPictureInPicture()}catch{}else LX.toast('Mini player não disponível neste navegador.')};q('#speed').onclick=()=>{speedIndex=(speedIndex+1)%speeds.length;v.playbackRate=speeds[speedIndex];q('#speed').textContent=`${speeds[speedIndex]}x`;showControls()};q('#subs')?.addEventListener('click',()=>{subIndex++;if(subIndex>=subtitleTracks.length)subIndex=-1;[...v.textTracks].forEach((tr,i)=>tr.mode=i===subIndex?'showing':'disabled');q('#subs').textContent=subIndex>=0?'CC✓':'CC';showControls()});q('#more').onclick=()=>{settings.classList.toggle('hide');showControls()};q('#restart').onclick=()=>{v.currentTime=0;saveProgress(id,0,0,v.duration||0,context,ep);settings.classList.add('hide');v.play().catch(()=>{})};q('#native').onclick=toggleNative;q('#reload').onclick=async()=>{settings.classList.add('hide');await setSource(v.currentTime||0)};q('#closeSettings').onclick=()=>settings.classList.add('hide');q('#retry').onclick=async()=>{await setSource(v.currentTime||0)};
 let touchRevealed=false,lastPointer=null;
 stage.addEventListener('pointerdown',e=>{if(e.pointerType!=='touch')return;touchRevealed=e.target===v&&!v.paused&&root.classList.contains('idle');showControls()},{passive:true});
 stage.addEventListener('pointermove',e=>{if(e.pointerType==='touch')return;const point={x:e.clientX,y:e.clientY};if(lastPointer&&Math.hypot(point.x-lastPointer.x,point.y-lastPointer.y)<4)return;lastPointer=point;showControls()},{passive:true});
 stage.addEventListener('click',e=>{if(e.target!==v)return;if(touchRevealed){touchRevealed=false;showControls();return}if(root.classList.contains('idle'))showControls();else togglePlay()},{passive:true});
 stage.addEventListener('pointercancel',()=>{touchRevealed=false},{passive:true});
 stage.addEventListener('focusin',e=>{if(e.target.matches?.('button,input'))showControls()});
 progress.addEventListener('input',showControls);
 q('#closeSettings').onclick=()=>{settings.classList.add('hide');showControls()};
 q('#restart').addEventListener('click',showControls);
 stage.ondblclick=e=>{if(e.target.closest?.('button,input'))return;const b=stage.getBoundingClientRect();e.clientX<b.left+b.width/2?seek(-10):seek(10)};
 ...
 activePlayerCleanup=()=>{clearTimeout(hideTimer);clearTimeout(driveFallbackTimer);if(driveFrame)try{driveFrame.src='about:blank'}catch{};window.removeEventListener('online',network);window.removeEventListener('offline',network);try{if(currentObjectUrl?.startsWith('blob:'))URL.revokeObjectURL(currentObjectUrl)}catch{}activeVideoEl=null;host.remove();document.body.classList.remove('lx-player-open')};
  setUI();showControls();await setSource(0,true);
}
...
const musicStreamFallbackCache=new Map();
const musicSourceFailures=new Map();
function cacheMusicSource(ref,value){if(!ref||!value)return value;musicSourceCache.set(String(ref),{value,until:Date.now()+(String(ref).startsWith('cloud:')?10:20)*60*1000});return value}
function cachedMusicSource(ref){const entry=ref?musicSourceCache.get(String(ref)):null;if(!entry)return null;if(entry.until<Date.now()){musicSourceCache.delete(String(ref));return null}return entry.value}
async function fetchMusicTicket(ref,expires=7200){
 ...
 const timed=(promise,ms=6500)=>Promise.race([Promise.resolve(promise),new Promise((_,reject)=>setTimeout(()=>reject(new Error('MEDIA_TIMEOUT')),ms))]);
 // Supabase Storage serves signed MP3s directly to the native audio element. Keep the
 // range-aware Edge proxy available if a particular browser cannot use its Storage URL.
 try{
  const sessionRes=await timed(db.auth.getSession(),3000),access=sessionRes?.data?.session?.access_token||'';
  if(access){
   const invoked=await timed(db.functions.invoke('lx-media-ticket',{body:{key,mediaKey:key,expires},headers:{Authorization:`Bearer ${access}`}}),6500);
   const data=invoked?.data,error=invoked?.error;
   if(!error&&(data?.streamUrl||data?.url||data?.signedUrl)){
    const url=data.streamUrl||data.url||data.signedUrl;
    if(data?.signedUrl)musicSignedFallbackCache.set(key,data.signedUrl);
    if(data?.streamUrl)musicStreamFallbackCache.set(key,data.streamUrl);
    musicSourceFailures.delete(key);return data.signedUrl||url
   }
   if(error||data?.error)musicSourceFailures.set(key,new Error(data?.message||data?.error||`Ticket HTTP ${error?.context?.status||'indisponível'}`));
  }
 }catch(error){musicSourceFailures.set(key,error);console.warn('LX media stream ticket failed',error)}
 try{
  const signed=await timed(db.storage.from(bucket).createSignedUrl(path,7200),5000);
  if(!signed?.error&&signed?.data?.signedUrl){musicSourceFailures.delete(key);musicSignedFallbackCache.set(key,signed.data.signedUrl);return signed.data.signedUrl}
  if(signed?.error)musicSourceFailures.set(key,signed.error);
 }catch(error){musicSourceFailures.set(key,error);console.warn('LX direct signed music URL failed',error)}
 return null
}
...
function musicDeadline(promise,ms,label){let timer;return Promise.race([Promise.resolve(promise),new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error(label)),ms)})]).finally(()=>clearTimeout(timer))}
async function prewarmMusicRef(ref){
 ...
 const task=(async()=>{
  ...
  if(key.startsWith('cloud:')){
   ...
   try{const value=await musicDeadline(S.getMedia(key),6500,'MEDIA_LOCAL_TIMEOUT');if(value)return cacheMusicSource(key,value)}catch(e){console.warn('LX cloud music fallback',e)}
   return null
  }
  try{const value=await musicDeadline(S.getMedia(key),6500,'MEDIA_LOCAL_TIMEOUT');return value?cacheMusicSource(key,value):null}catch(e){console.warn('LX prewarm music source',e);return null}
...
function prewarmMusicCatalog(list=[]){
 // Warm only a few private MP3s. A ready URL lets the first mobile tap
 // reach the native audio element before its user activation expires.
 const refs=[];for(const x of list)for(const ref of musicRefsForContent(x))if(String(ref).startsWith('cloud:')&&!refs.includes(ref))refs.push(ref);
 return Promise.allSettled(refs.slice(0,8).map(ref=>prewarmMusicRef(ref)));
}
...
function unlockMusicGesture(){
 // The real <audio> must use the tap. A silent second player can consume it.
 LX.audioEffects?.resume?.()?.catch?.(()=>{});
}
...
async function openMusicProvider(){const dock=$('musicDock'),panel=$('musicProviderPanel');if(!dock?.classList.contains('external-provider')||!panel)return false;const opening=!dock.classList.contains('provider-open');if(!opening&&providerPlayback.kind==='youtube'){youtubeCommand('pauseVideo');providerPlayback.paused=true;updateMusicUI()}dock.classList.toggle('provider-open',opening);panel.setAttribute('aria-hidden',String(!opening));const btn=$('musicProviderToggle');if(btn){btn.classList.remove('hidden');btn.title=opening?'Ocultar player oficial':'Mostrar player oficial';btn.setAttribute('aria-label',btn.title)}return opening}
function youtubeCommand(func,args=[]){const frame=$('musicProviderFrame');if(!frame?.contentWindow||!frame.src)return false;try{const origin=new URL(frame.src).origin;frame.contentWindow.postMessage(JSON.stringify({event:'command',func,args}),origin);return true}catch{return false}}
...
async function setupMusicEmbed(desc,autoplay=false,expectedTicket=musicLoadTicket){
 ...
 if(frame){
  ...
  frame.src=src;frame.dataset.provider=kind;frame.style.height=(kind==='youtube'?200:Math.max(152,Number(desc.embedHeight)||180))+'px';frame.classList.remove('hidden');
...
function openMusicAlbum(id){const grouped=musicAutoAlbums(musicCatalog()).find(row=>row.itemIds.includes(String(id)));if(grouped)return openMusicAutoAlbum(grouped.id);const x=D.catalog().find(i=>String(i.id)===String(id));if(!x)return;const tracks=(x.tracks?.length?x.tracks:((x.mediaKey||x.authorizedAudioUrl)?[{title:x.title,mediaKey:x.mediaKey||x.authorizedAudioUrl,duration:0}]:[])),provider=musicProvider(x),totalSecs=tracks.reduce((a,t)=>a+(+t.duration||0),0),saved=D.myList().some(z=>String(z)===String(id));$('modal').innerHTML=`<button class="close-btn" onclick="LX.ui.close()">×</button><div class="lx-album-page lx-album-page-v256 lx-album-page-v260"><section class="lx-album-hero"><div class="lx-album-cover" style="background-image:url('${x.cover||x.banner||''}')"></div><div class="lx-album-copy"><span class="eyebrow">LX MUSIC · ${tracks.length>1?'ÁLBUM':'SINGLE'}</span><h2>${esc(x.title)}</h2><p>${esc(x.artist||'LX Music')} · ${x.year||'2026'} · ${esc(x.genre||'Música')}</p><div class="lx-album-badges"><span>${LX.musicProviderBrandHtml(x,true)}</span><span>${tracks.length} ${tracks.length===1?'faixa':'faixas'}</span><span>${totalSecs?LX.fmt(totalSecs):'Ao vivo'}</span></div><div class="hero-actions"><button class="primary-btn" onclick="LX.music(${id},0)">▶ Reproduzir</button><button class="secondary-btn" onclick="LX.musicShuffleAlbum(${id})">⇄ Embaralhar</button><button id="lxAlbumSave" class="secondary-btn ${saved?'active':''}" onclick="LX.musicToggleSaved(${id})">${saved?'♥ Salvo':'♡ Salvar'}</button>${x.lyrics||x.desc?'<button class="secondary-btn" onclick="LX.openMusicLyrics('+id+',0)">≡ Letra</button>':''}</div></div></section><div class="lx-track-list lx-track-list-v256"><div class="lx-track-list-head"><span>#</span><span>Título</span><span>Duração</span><span></span></div>${tracks.map((t,i)=>`<button class="lx-track-row" data-music-id="${id}" onclick="LX.music(${id},${i})"><b>${i+1}</b><span><strong>${esc(t.title||`Faixa ${i+1}`)}</strong><small>${esc(t.artist||x.artist||'LX Music')}</small></span><em>${LX.fmt(t.duration||0)}</em><i data-music-play-icon>▶</i></button>`).join('')||'<div class="notice">Nenhuma faixa enviada ainda.</div>'}</div></div>`;$('overlay').classList.remove('hidden');queueMicrotask(syncMusicCardState)}
async function toggleCurrentMusic(){
 unlockMusicGesture();const t=currentMusic();if(!t)return;
 if($('musicAudio')?.dataset.loadingTrackTicket){setMusicStatus('Preparando áudio…','loading');return}
 if(isExternalMusicTrack(t)){const d=providerPlayback.desc||musicEmbedDesc(t);if(!d){loadTrack(true);return}if(providerPlayback.kind==='spotify'){if(!spotifyController){setMusicStatus('Use os controles oficiais do Spotify','error');openMusicProvider();return}try{providerPlayback.paused?spotifyController.play():spotifyController.pause();setMusicStatus('Aguardando confirmação do Spotify…','ready')}catch(error){console.warn('LX Spotify play/pause',error);setMusicStatus('Toque no player oficial do Spotify','error');openMusicProvider()}return}if(providerPlayback.kind==='youtube'){if(providerPlayback.paused&&!$('musicDock')?.classList.contains('provider-open')){await openMusicProvider();await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))}if(youtubeCommand(providerPlayback.paused?'playVideo':'pauseVideo'))setMusicStatus('Aguardando confirmação do YouTube…','ready');else setMusicStatus('Player do YouTube indisponível · tente novamente','error');return}if(!$('musicDock')?.classList.contains('provider-open'))openMusicProvider();setMusicStatus('Use os controles do player oficial','ready');return}
 const a=$('musicAudio');if(!a)return;
 if(!a.src||a.error||a.networkState===3){await loadTrack(true);return}
 if(!a.paused){a.pause();return}
 try{await musicDeadline(a.play(),6500,'PLAY_TIMEOUT');setMusicStatus('Reproduzindo','ready')}catch(error){console.warn('LX manual play',error);await recoverNativeMusicSource(t,true,error)}updateMusicUI()
}
...
function listenerCollections(){const rows=S.read(S.keys.musicCollections,[]);return Array.isArray(rows)?rows:[]}
async function persistListenerCollections(rows,message='Coleção atualizada'){
 S.write(S.keys.musicCollections,rows);if(state.screen==='app'&&state.mode==='Ouvir')U.renderApp();
 const ok=await LX.cloud?.syncUserState?.();LX.toast(ok?`${message} na sua conta.`:`${message} neste aparelho. A nuvem ainda não confirmou; tente novamente com a conexão ativa.`);
}
function openMusicCollectionEditor(id=null,kind='playlist',initialTrack=null){
 const row=id?listenerCollections().find(c=>String(c.id)===String(id)):null,mode=row?.kind||kind,chosen=new Set((row?.items||[]).map(String));if(initialTrack)chosen.add(String(initialTrack));
 const available=musicCatalog(),modal=$('modal'),overlay=$('overlay');
 modal.onclick=null;modal.innerHTML=`<button type="button" class="close-btn" data-lx-collection-close aria-label="Fechar">×</button><div class="panel-page lx-collection-editor"><div class="panel-head"><div><span class="eyebrow">LX MUSIC</span><h2>${row?'Editar':mode==='album'?'Novo álbum pessoal':'Nova playlist'}</h2><p>Escolha músicas já publicadas no catálogo. Sua coleção fica na sua conta.</p></div></div><form id="lxCollectionForm"><label>Nome ${mode==='album'?'do álbum':'da playlist'}<input name="name" required maxlength="80" value="${esc(row?.title||'')}" placeholder="${mode==='album'?'Meu álbum':'Minha playlist'}"></label><div class="lx-collection-editor-list" role="group" aria-label="Escolher músicas"><input id="lxCollectionFilter" type="search" placeholder="Filtrar músicas" aria-label="Filtrar músicas">${available.map(item=>`<label data-lx-collection-label="${esc((item.title||'')+' '+(item.artist||''))}"><input type="checkbox" name="track" value="${esc(item.id)}" ${chosen.has(String(item.id))?'checked':''}><span>${esc(item.title)}<small>${esc(item.artist||'LX Music')}</small></span></label>`).join('')}</div><div class="lx-music-collection-actions"><button type="submit" class="primary-btn">Salvar ${mode==='album'?'álbum':'playlist'}</button></div></form></div>`;
 overlay.classList.remove('hidden');modal.querySelector('[data-lx-collection-close]').onclick=()=>U.close();
 modal.querySelector('#lxCollectionFilter').oninput=e=>{const term=String(e.target.value||'').toLocaleLowerCase('pt-BR');modal.querySelectorAll('[data-lx-collection-label]').forEach(label=>{label.hidden=!String(label.dataset.lxCollectionLabel).toLocaleLowerCase('pt-BR').includes(term)})};
 modal.querySelector('#lxCollectionForm').onsubmit=e=>{
  e.preventDefault();const form=e.currentTarget,name=form.elements.name.value.trim().slice(0,80),items=[...form.querySelectorAll('input[name="track"]:checked')].map(input=>Number(input.value)).filter(Number.isFinite).slice(0,250);
  if(!name)return form.elements.name.focus();const rows=listenerCollections(),now=Date.now();if(!row&&rows.length>=100){LX.toast('Limite de 100 coleções por conta.');return}
  const next=row?rows.map(c=>c.id===row.id?{...c,title:name,items,updatedAt:now}:c):[{id:(crypto.randomUUID?.()||`${now}-${Math.random().toString(36).slice(2)}`),kind:mode==='album'?'album':'playlist',title:name,items,createdAt:now,updatedAt:now},...rows];
  state.musicCollectionSelected=row?.id||next[0].id;U.close();persistListenerCollections(next,row?'Coleção atualizada':'Coleção criada');
 };
}
function openMusicCollectionPicker(trackId){
 const item=musicCatalog().find(x=>String(x.id)===String(trackId));if(!item){LX.toast('Música não encontrada no catálogo.');return}
 const modal=$('modal');modal.innerHTML=`<button type="button" class="close-btn" data-lx-collection-close aria-label="Fechar">×</button><div class="panel-page lx-collection-picker"><div class="panel-head"><div><span class="eyebrow">LX MUSIC</span><h2>Adicionar à coleção</h2><p>${esc(item.title)} · ${esc(item.artist||'LX Music')}</p></div></div><div class="lx-collection-picker-list">${listenerCollections().map(row=>`<button type="button" data-lx-collection-pick="${esc(row.id)}"><strong>${esc(row.title)}</strong><small>${row.kind==='album'?'Álbum pessoal':'Playlist'} · ${(row.items||[]).length} músicas</small></button>`).join('')}</div><div class="lx-music-collection-actions"><button type="button" data-lx-collection-create="playlist">＋ Nova playlist</button><button type="button" data-lx-collection-create="album">＋ Novo álbum</button></div></div>`;
 $('overlay').classList.remove('hidden');modal.querySelector('[data-lx-collection-close]').onclick=()=>U.close();
 modal.onclick=e=>{const hit=e.target.closest('button');if(!hit)return;if(hit.dataset.lxCollectionPick){const rows=listenerCollections();let added=false;const next=rows.map(c=>{if(c.id!==hit.dataset.lxCollectionPick)return c;if((c.items||[]).some(key=>String(key)===String(trackId))){LX.toast('Esta música já está nessa coleção.');return c}added=true;return {...c,items:[...(c.items||[]),Number(trackId)].slice(0,250),updatedAt:Date.now()}});if(added){U.close();persistListenerCollections(next,'Música adicionada')}return}if(hit.dataset.lxCollectionCreate){const kind=hit.dataset.lxCollectionCreate;openMusicCollectionEditor(null,kind,trackId)}};
}
function playMusicCollection(id,trackId=null){const row=listenerCollections().find(c=>String(c.id)===String(id));if(!row)return LX.toast('Coleção indisponível.');const ids=(row.items||[]).filter(key=>musicCatalog().some(item=>String(item.id)===String(key)));const start=trackId&&ids.some(key=>String(key)===String(trackId))?trackId:ids[0];if(!start)return LX.toast('Adicione músicas do catálogo antes de reproduzir.');music(start,0,true,ids)}
function removeMusicCollectionTrack(id,trackId){const rows=listenerCollections();if(!rows.some(c=>String(c.id)===String(id)))return;persistListenerCollections(rows.map(c=>String(c.id)===String(id)?{...c,items:(c.items||[]).filter(key=>String(key)!==String(trackId)),updatedAt:Date.now()}:c),'Música removida')}
function deleteMusicCollection(id){const row=listenerCollections().find(c=>String(c.id)===String(id));if(!row||!window.confirm(`Excluir “${row.title}”? As músicas continuarão no catálogo.`))return;state.musicCollectionSelected=null;persistListenerCollections(listenerCollections().filter(c=>String(c.id)!==String(id)),'Coleção excluída')}
LX.openMusicCollectionEditor=openMusicCollectionEditor;LX.openMusicCollectionPicker=openMusicCollectionPicker;LX.playMusicCollection=playMusicCollection;LX.removeMusicCollectionTrack=removeMusicCollectionTrack;LX.deleteMusicCollection=deleteMusicCollection;
function music(id,index=0,autoplay=true,queueIds=null){
 try{
 if(autoplay)unlockMusicGesture();
 const x=D.catalog().find(i=>String(i.id)===String(id));if(!x){LX.toast('Música indisponível no catálogo. Atualize a página.');return}
 const tracks=x.tracks?.length?x.tracks:[{number:1,title:x.title,duration:0,mediaKey:x.mediaKey||x.authorizedAudioUrl||x.externalMusicUrl||'',sourceMediaKey:x.mediaKey||''}];
 const nextIndex=Math.max(0,Math.min(index,tracks.length-1)),same=String(currentMusic()?.contentId)===String(id)&&currentMusic()?.index===nextIndex&&!$('musicDock')?.classList.contains('hidden');if(same&&!queueIds){if(autoplay)toggleCurrentMusic();return}
 const genre=state.musicGenre&&state.musicGenre!=='Todos'&&musicHasGenre(x,state.musicGenre)?state.musicGenre:musicGenresOf(x)[0];
 const peers=musicCatalog().filter(item=>String(item.id)!==String(id)&&musicHasGenre(item,genre));
 const selectedIds=Array.isArray(queueIds)?[...new Set(queueIds.map(String))]:null,queueItems=selectedIds?[x,...selectedIds.filter(key=>key!==String(id)).map(key=>musicCatalog().find(item=>String(item.id)===key)).filter(Boolean)]:[x,...peers];
 D.track('music',{id,title:x.title});state.musicQueue=queueItems.flatMap(item=>{
  const list=item.tracks?.length?item.tracks:[{number:1,title:item.title,mediaKey:item.mediaKey||item.authorizedAudioUrl||item.externalMusicUrl||''}];
  return list.map((track,i)=>{const sourceMediaKey=track.sourceMediaKey||track.mediaKey||track.url||item.sourceMediaKey||item.mediaKey||item.externalMusicUrl||'',playbackKey=track.authorizedAudioUrl||track.authorizedStreamUrl||item.authorizedAudioUrl||item.authorizedStreamUrl||track.fullMediaKey||track.audioKey||track.audioUrl||track.mediaKey||track.url||item.fullMediaKey||item.audioKey||item.audioUrl||item.mediaKey||'';return {...track,sourceMediaKey,mediaKey:playbackKey,contentId:item.id,index:i,title:String(track.title||item.title||'Faixa').trim()||'Faixa',artist:String(track.artist||item.artist||'LX Music').trim()||'LX Music',youtubeThumbnail:track.youtubeThumbnail||item.youtubeThumbnail||'',spotifyArtwork:track.spotifyArtwork||item.spotifyArtwork||'',cover:String(track.cover||item.cover||LX.artwork.url({...track,sourceMediaKey},item)||LX.artwork.fallback),album:track.album||item.album||item.title,albumCover:item.cover||'',contentCover:item.cover||''}})
 });state.musicIndex=nextIndex;$('musicDock').classList.remove('hidden');
 ...
 loadTrack(autoplay).catch(error=>{console.error('LX music selection',error);setMusicStatus('Falha ao abrir: '+musicErrorText(error),'error');LX.toast('Falha ao abrir esta música.');}).finally(()=>rememberMusicStart(id,nextIndex))
 }catch(error){console.error('LX music selection',error);setMusicStatus('Falha ao abrir: '+musicErrorText(error),'error');LX.toast('Falha ao abrir esta música: '+musicErrorText(error))}
}
let musicLoadTicket=0,musicReadyPromise=Promise.resolve(false),musicLoadGuard=null,musicStallGuard=null;
...
async function recoverNativeMusicSource(t,autoplay=true,previousError=null,quiet=false,expectedTicket=musicLoadTicket){
 ...
 const trySource=async(media)=>{
  ...
  try{
   ...
   if(!current())return false;
   cacheMusicSource(ref,media);providerPlayback={kind:'native',paused:a.paused,desc:null};setMusicStatus(a.paused?'Pronta · toque em ▶':'Reproduzindo','ready');return true
...
async function loadTrack(autoplay=false){
 ...
 try{
  for(const ref of [...new Set(refs)]){
   ...
   let media=cachedMusicSource(ref);if(!media&&desc?.kind==='direct'&&desc.src)media=cacheMusicSource(ref,desc.src);if(!media)media=await prewarmMusicRef(ref);if(ticket!==musicLoadTicket)return false;if(!media){lastError=musicSourceFailures.get(String(ref))||lastError;continue}
   try{
    ...
    a.dataset.sourceRef=String(ref);a.crossOrigin='anonymous';a.preload='auto';a.src=String(source);a.load();
    const started=autoplay?a.play().then(()=>null,error=>error):null;
    await waitForMusicReady(a,7000);if(ticket!==musicLoadTicket)return false;
    if(started){const playError=await musicDeadline(started,7000,'PLAY_TIMEOUT');if(playError)throw playError}
    updateMusicDuration(a);providerPlayback={kind:'native',paused:a.paused,desc:null};setMusicStatus(a.paused?'Pronta · toque em ▶':'Reproduzindo','ready');updateMusicUI();return true
  ...
  if(embed){await setupMusicEmbed(embed,autoplay,ticket);if(ticket!==musicLoadTicket)return false;setMusicStatus('Use o player oficial para reproduzir','ready');LX.syncMusicProviderBrand?.(t);syncMusicMediaSession(t);updateMusicUI();return true}
  a.removeAttribute('src');a.load();$('musicDuration').textContent='—';setMusicStatus('Áudio indisponível: '+musicErrorText(lastError),'error');if(autoplay&&!cloudRef)LX.toast('Falha no áudio: '+musicErrorText(lastError));updateMusicUI();return false
 }catch(error){if(ticket!==musicLoadTicket)return false;console.warn('LX music load',error);$('musicDuration').textContent='—';setMusicStatus('Falha no áudio: '+musicErrorText(error),'error');updateMusicUI();return false}
 finally{if(ticket===musicLoadTicket)clearTimeout(musicLoadGuard);if(a.dataset.loadingTrackTicket===String(ticket))delete a.dataset.loadingTrackTicket}
}
function updateMusicUI(){const a=$('musicAudio'),t=currentMusic(),paused=isExternalMusicTrack(t)?providerPlayback.paused:a.paused,status=$('musicPlaybackKind');$('musicPlay').innerHTML=LX.artwork.icon(paused?'play':'pause');$('musicPlay').title=status?.classList.contains('is-error')?'Tentar novamente':status?.classList.contains('is-loading')?'Carregando áudio':'Reproduzir ou pausar';$('musicPlay').setAttribute('aria-label',$('musicPlay').title);$('musicVolume').disabled=['spotify','embed'].includes(providerPlayback.kind);$('musicVolume').title=providerPlayback.kind==='spotify'?'Ajuste o volume no dispositivo ou no Spotify':'Volume';$('musicShuffle').classList.toggle('active',musicShuffleMode);$('musicShuffle').setAttribute('aria-pressed',String(musicShuffleMode));$('musicRepeat').classList.toggle('active',musicRepeatMode>0);$('musicRepeat').innerHTML=LX.artwork.icon('repeat')+(musicRepeatMode===2?'<sup>1</sup>':'');$('musicRepeat').setAttribute('aria-pressed',String(musicRepeatMode>0));try{if('mediaSession'in navigator)navigator.mediaSession.playbackState=t?(paused?'paused':'playing'):'none'}catch{}syncMusicCardState();syncMusicSaveUI();syncRightMusicRail?.()}
function seekMusicProvider(percent){const duration=providerPlayback.kind==='spotify'?spotifyPlayback.duration/1000:providerPlayback.duration||Number(currentMusic()?.duration)||0;if(!duration)return;const time=Math.max(0,Math.min(duration,Number(percent)/100*duration));if(providerPlayback.kind==='youtube')youtubeCommand('seekTo',[time,true]);else if(providerPlayback.kind==='spotify'&&spotifyController?.seek)spotifyController.seek(Math.floor(time));else return;$('musicTime').textContent=LX.fmt(time)}
...
$('musicPlay').onclick=()=>toggleCurrentMusic();$('musicPrev').onclick=musicPrev;$('musicNext').onclick=()=>musicNext(true);$('musicProviderPrev')&&($('musicProviderPrev').onclick=musicPrev);$('musicProviderNext')&&($('musicProviderNext').onclick=()=>musicNext(true));$('musicProviderToggle')&&($('musicProviderToggle').onclick=()=>LX.openMusicProvider());$('musicClose').onclick=()=>{try{spotifyController?.pause?.()}catch{};$('musicAudio').pause();resetMusicProvider();$('musicDock').classList.add('hidden');spotifyPlayback.paused=true;syncMusicCardState();document.dispatchEvent(new Event('lx:music-closed'))};$('musicShuffle').onclick=()=>{musicShuffleMode=!musicShuffleMode;updateMusicUI()};$('musicRepeat').onclick=()=>{musicRepeatMode=(musicRepeatMode+1)%3;updateMusicUI()};$('musicQueueBtn').onclick=openMusicQueue;$('musicCoverBtn').onclick=()=>{const t=currentMusic();if(t)LX.openMusicAlbum(t.contentId)};$('musicVolume').oninput=()=>{const v=+$('musicVolume').value;$('musicAudio').volume=v;if(providerPlayback.kind==='youtube')youtubeCommand('setVolume',[Math.round(v*100)])};$('musicAudio').onerror=()=>{const a=$('musicAudio'),t=currentMusic();if(!t||!a?.src||a.dataset.loadingTrackTicket||isExternalMusicTrack(t))return;console.warn('LX music element error',a.error);musicSourceCache.delete(String(a.dataset.sourceRef||''));$('musicDuration').textContent='—';setMusicStatus('Fonte interrompida: '+musicErrorText(a.error),'error');updateMusicUI()};$('musicAudio').onloadedmetadata=()=>{const a=$('musicAudio');updateMusicDuration(a);syncMusicMediaSession(currentMusic());syncMusicMediaPosition(a,true)};$('musicAudio').ondurationchange=()=>updateMusicDuration();$('musicAudio').onplay=()=>{clearTimeout(musicStallGuard);setMusicStatus('Reproduzindo','ready');syncMusicMediaSession(currentMusic());syncMusicMediaPosition($('musicAudio'),true);updateMusicUI()};$('musicAudio').onpause=()=>{saveMusicProgress();syncMusicMediaPosition($('musicAudio'),true);updateMusicUI()};$('musicAudio').ontimeupdate=()=>{const a=$('musicAudio');if(a.currentTime>0){clearTimeout(musicStallGuard);if($('musicPlaybackKind')?.classList.contains('is-loading'))setMusicStatus('Reproduzindo','ready')}const duration=updateMusicDuration(a);if(duration>0){$('musicProgress').value=Math.max(0,Math.min(100,a.currentTime/duration*100));$('musicTime').textContent=LX.fmt(a.currentTime);syncMusicMediaPosition(a);if(Date.now()-musicSaveAt>5000){musicSaveAt=Date.now();saveMusicProgress()}}};$('musicAudio').onended=()=>{saveMusicProgress();if(musicRepeatMode===2){$('musicAudio').currentTime=0;$('musicAudio').play().catch(()=>{})}else if(musicRepeatMode===1||state.musicIndex<state.musicQueue.length-1)musicNext(true);else updateMusicUI()};$('musicAudio').onwaiting=()=>{const a=$('musicAudio'),ticket=musicLoadTicket;clearTimeout(musicStallGuard);musicStallGuard=setTimeout(()=>{if(ticket===musicLoadTicket&&!a.paused&&a.readyState<3){setMusicStatus('Conexão interrompida · toque em ▶ para tentar novamente','error');updateMusicUI()}},8000)};$('musicProgress').oninput=()=>{if(isExternalMusicTrack()){seekMusicProvider($('musicProgress').value);return}const a=$('musicAudio'),duration=updateMusicDuration(a);if(duration>0){a.currentTime=+$('musicProgress').value/100*duration;syncMusicMediaPosition(a,true)}};
document.addEventListener('lx:music-artwork-updated',event=>{if(currentMusic()!==event.detail?.item)return;const t=event.detail.item;setMusicDockArtwork(t,currentMusicContent()||{});$('musicTitle').textContent=t.title||'Faixa';$('musicArtist').textContent=t.artist||'LX Music';LX.syncMusicProviderBrand?.(t);$('musicDuration').textContent=LX.fmt(t.duration||0);syncMusicMediaSession(t);syncMusicCardState();document.dispatchEvent(new Event('lx:music-changed'))});$('musicLikeBtn')&&($('musicLikeBtn').onclick=musicToggleSavedCurrent);$('musicCollectionBtn')&&($('musicCollectionBtn').onclick=()=>{const t=currentMusic();if(t)openMusicCollectionPicker(t.contentId);else LX.toast('Escolha uma música primeiro.')});$('musicLyricsBtn')&&($('musicLyricsBtn').onclick=()=>openMusicLyrics());
...
function uiPrefs(){return {...{font:'modern',density:'balanced',posterSize:'small',navigation:'top'},...(S.read(S.keys.uiPrefs,{})||{})}}
function applyTheme(){const theme=S.read(S.keys.theme,'dark'),accent=S.read(S.keys.accent,'#42a5ff'),layout=S.read(S.keys.layoutMode,'cinema'),motion=S.read(S.keys.motion,'full'),prefs=uiPrefs(),navigation=prefs.navigation==='side'?'side':'top';document.documentElement.dataset.theme=theme;document.body.dataset.theme=theme;document.body.classList.toggle('light',theme==='light');document.documentElement.dataset.font=prefs.font||'modern';document.body.dataset.font=prefs.font||'modern';document.documentElement.dataset.density=prefs.density||'balanced';document.body.dataset.density=prefs.density||'balanced';document.documentElement.dataset.posterSize=prefs.posterSize||'small';document.body.dataset.posterSize=prefs.posterSize||'small';document.documentElement.dataset.navPlacement=navigation;document.body.dataset.navPlacement=navigation;document.body.classList.toggle('lx-nav-side',navigation==='side');document.documentElement.style.setProperty('--accent',accent);document.documentElement.style.setProperty('--accent2',accent);document.body.classList.toggle('app-mode',layout==='app');document.body.classList.toggle('compact-mode',layout==='compact');document.body.classList.toggle('motion-reduced',motion==='reduced');document.documentElement.dataset.layout=layout;document.documentElement.dataset.motion=motion;const meta=document.querySelector('meta[name=theme-color]');if(meta)meta.content=theme==='light'?'#f4f6f8':theme==='oled'?'#000000':'#03070c'}
...
(()=>{
  ...
  function openProfileMenu(){
    ...
    m.querySelectorAll('[data-act]').forEach(b=>b.onclick=()=>{const a=b.dataset.act;closeProfileMenu();if(a==='profile')LX.openProfile?.();else if(a==='privacy')openPrivacyCenter();else if(a==='notifications')LX.openNotifications?.();else if(a==='appearance')LX.openTheme?.();else if(a==='community')LX.social?.open?.('friends');else if(a==='premium')LX.openPremium?.();else if(a==='install')window.LXPWA?.install?.();else if(a==='admin')LX.openAdmin?.();else if(a==='logout')$('profileLogout')?.click()});
  }
  ...
  async function noticeItems(){
    const rows=(D.notices()||[]).map(n=>({cat:classifyNotice(n),icon:classifyNotice(n)==='content'?'▶':'✦',title:n.title||'Atualização',text:n.text||n.message||'',time:n.time||'Agora',tag:classifyNotice(n)==='content'?'Conteúdo':'Sistema',read:!!n.read}));
  ...
  async function openNotifications38(tab='all'){
    const modal=$('modal');if(!modal)return; $('overlay')?.classList.remove('hidden');modal.innerHTML='<button class="close-btn" onclick="LX.ui.close()">×</button><div class="panel-page lx-notice-center"><div class="lx-notice-head"><span class="lx-notice-emblem" aria-hidden="true">◌</span><div><span class="eyebrow">SUA CENTRAL</span><h2>Notificações</h2><p>Novidades do catálogo, recados e conversas.</p></div><button class="glass-btn" id="lxReadAll38" type="button">Marcar tudo como lido</button></div><div class="lx-notice-tabs" id="lxNoticeTabs" role="tablist" aria-label="Filtrar notificações"></div><div id="lxNoticeBody" class="lx-notice-list" aria-live="polite"><div class="lx-search-skeleton"><i class="lx-skeleton"></i><i class="lx-skeleton"></i></div></div></div>';
    let items=[];try{items=await noticeItems()}catch(error){console.warn('LX notifications',error);$('lxNoticeBody').innerHTML='<div class="lx-notice-empty">Não foi possível carregar os avisos agora. Feche e tente novamente.</div>';return}
    const tabs=[['all','Tudo'],['community','Comunidade'],['content','Conteúdo'],['system','Sistema']];
    const draw=t=>{if(!document.getElementById('lxNoticeTabs'))return;const filtered=t==='all'?items:items.filter(x=>x.cat===t),unread=items.filter(x=>!x.read).length;$('lxNoticeTabs').innerHTML=tabs.map(([k,n])=>`<button type="button" role="tab" aria-selected="${k===t}" class="${k===t?'active':''}" data-lx-nt="${k}">${n}<small>${k==='all'?items.length:items.filter(x=>x.cat===k).length}</small></button>`).join('');$('lxNoticeBody').innerHTML=filtered.length?filtered.map(x=>`<article class="lx-notice-item ${x.read?'is-read':'is-unread'} ${x.cat==='community'?'is-actionable':''}" data-lx-kind="${esc(x.cat||'system')}"><span class="lx-notice-icon" aria-hidden="true">${esc(x.icon||'✦')}</span><div><strong>${esc(x.title)}</strong><p>${esc(x.text)}</p><small>${esc(x.time)}</small></div><span class="lx-notice-badge">${esc(x.tag||'Aviso')}</span></article>`).join(''):`<div class="lx-notice-empty"><span aria-hidden="true">✓</span><strong>Tudo em dia</strong><p>${t==='all'?'Seus avisos aparecerão aqui quando houver novidades.':'Nenhuma notificação nesta categoria.'}</p></div>`;$('lxReadAll38').disabled=!unread;$('lxNoticeTabs').querySelectorAll('[data-lx-nt]').forEach(b=>b.onclick=()=>draw(b.dataset.lxNt));$('lxNoticeBody').querySelectorAll('.is-actionable').forEach(b=>b.onclick=()=>{LX.ui.close();LX.social?.open?.('chats')})};
    draw(tab);$('lxReadAll38').onclick=async()=>{try{LX.readAll?.();await LX.chat?.markNotificationsRead?.();items=await noticeItems();draw(tab)}catch(error){console.warn('LX notification read',error);LX.toast?.('Não foi possível marcar todas as notificações.')}};
  }
  LX.noticeCenter={open:openNotifications38};
  V.openNotifications=()=>LX.openNotifications?.();
  ...
  function scheduleDecorate(){if(V.decorateQueued)return;V.decorateQueued=true;requestAnimationFrame(()=>{V.decorateQueued=false;decorateApp();enhanceProfileModal();decorateAppearance();enhanceSocialProfile()})}
  ...
  const notify=$('notifyBtn');if(notify)notify.title='Central de notificações';
...
(()=>{
  ...
  const pageCaps={dashboard:['owner','administrator','editor','moderator'],library:['owner','administrator','editor'],movies:['owner','administrator','editor'],series:['owner','administrator','editor'],music:['owner','administrator','editor'],books:['owner','administrator','editor'],live:['owner','administrator','editor'],carousel:['owner','administrator','editor'],lxai:['owner','administrator','editor','moderator'],importer:['owner','administrator','editor'],uploads:['owner','administrator','editor'],requests:['owner','administrator','moderator'],community:['owner','administrator','moderator'],admins:['owner'],premium:['owner','administrator'],analytics:['owner','administrator'],notifications:['owner','administrator','moderator'],appearance:['owner','administrator'],settings:['owner','administrator']};
  ...
  let lastUser='';
  function heartbeat(){
    const current=uid();if(lastUser&&current!==lastUser){$('musicAudio')?.pause();if($('musicAudio'))$('musicAudio').currentTime=0;state().musicQueue=[];state().musicIndex=0;$('musicDock')?.classList.add('hidden')}lastUser=current;if(current)subscribeStreak();applyAdminPermissions();renderAdminTeam();syncBranding();$('lxStreakIndicator')?.remove();$('lxStreakWelcome')?.remove();
  }
  ...
  async function analyzeRows(rows){
    ...
    try{
      for(let i=0;i<rows.length;i+=25){
        const sourceChunk=rows.slice(i,i+25),transport=sourceChunk.map(({file,localMeta,localMediaKey,...rest})=>rest);let returned=[];try{const d=await universalHub('catalog_match_batch',{items:transport,type:'auto'});returned=d.items||[]}catch(remoteError){console.warn('Catálogo remoto indisponível; mantendo tags e nomes locais.',remoteError)}
      ...
      toast('✓ Áudios carregados. Confira apenas os dados que faltaram.')
...
(()=>{
 ...
 async function readMp4Tags(file){const found={},unit=2*1024*1024,ranges=[[0,Math.min(unit,file.size)]];if(file.size>unit)ranges.push([Math.max(unit,file.size-unit),file.size]);const four=(a,i)=>String.fromCharCode(...a.slice(i,i+4)),uint=(a,i)=>((a[i]*16777216)+(a[i+1]<<16)+(a[i+2]<<8)+a[i+3])>>>0;
  for(const [start,end] of ranges){const a=new Uint8Array(await file.slice(start,end).arrayBuffer());for(let i=4;i<a.length-12;i++){if(four(a,i)!=='ilst')continue;const boxStart=i-4,size=uint(a,boxStart),limit=boxStart+size;if(size<16||limit>a.length)continue;for(let p=i+4;p+16<=limit;){const len=uint(a,p),kind=four(a,p+4);if(len<16||p+len>limit)break;for(let d=p+8;d+16<=p+len;){const n=uint(a,d);if(n<16||d+n>p+len)break;if(four(a,d+4)==='data'){const bytes=a.slice(d+16,d+n),value=new TextDecoder('utf-8').decode(bytes).replace(/\0/g,'').trim();if(kind==='©nam')found.title=value;else if(kind==='©ART'||kind==='aART')found.artist=value;else if(kind==='©alb')found.album=value;else if(kind==='©gen')found.genre=value;else if(kind==='©day')found.year=(value.match(/(?:19|20)\d{2}/)||[])[0]||'';else if(kind==='trkn'&&bytes.length>=4)found.trackNumber=(bytes[2]<<8)|bytes[3];else if(kind==='covr'&&bytes.length&&bytes.length<1500000)found.cover=await dataUrl(new Blob([bytes],{type:bytes[0]===0x89?'image/png':'image/jpeg'}));break}d+=n}p+=len}if(Object.keys(found).length)return found}}
  return found
 }
 async function readId3(file){const out={title:'',artist:'',album:'',year:'',genre:'',cover:'',duration:0,trackNumber:0};if(!file||!/\.(?:mp3|m4a|aac|flac|wav|ogg|opus)$/i.test(file.name||''))return out;
  if(/\.(?:mp3|aac)$/i.test(file.name||'')){try{const head=new Uint8Array(await file.slice(0,10).arrayBuffer());if(String.fromCharCode(...head.slice(0,3))==='ID3'){const ver=head[3],tagSize=syncsafe(head[6],head[7],head[8],head[9]),max=Math.min(file.size,10+tagSize,5*1024*1024),buf=new Uint8Array(await file.slice(0,max).arrayBuffer());let pos=10;while(pos+10<=buf.length){const fid=String.fromCharCode(...buf.slice(pos,pos+4));if(!/^[A-Z0-9]{4}$/.test(fid))break;const size=ver===4?syncsafe(buf[pos+4],buf[pos+5],buf[pos+6],buf[pos+7]):u32(buf[pos+4],buf[pos+5],buf[pos+6],buf[pos+7]);if(!size||pos+10+size>buf.length)break;const body=buf.slice(pos+10,pos+10+size),enc=body[0],text=()=>clean(decoder(enc,body.slice(1))).replace(/^\uFEFF/,'');if(fid==='TIT2')out.title=text();else if(fid==='TPE1')out.artist=text();else if(fid==='TALB')out.album=text();else if(fid==='TYER'||fid==='TDRC')out.year=(text().match(/(?:19|20)\d{2}/)||[])[0]||'';else if(fid==='TCON')out.genre=text().replace(/^\(\d+\)\s*/,'');else if(fid==='TRCK')out.trackNumber=Number((text().match(/^\d+/)||[])[0]||0);else if(fid==='APIC'&&body.length>12&&!out.cover){let i=1;while(i<body.length&&body[i]!==0)i++;const mime=clean(decoder(0,body.slice(1,i)))||'image/jpeg';i++;i++;if(enc===1||enc===2){while(i+1<body.length&&(body[i]!==0||body[i+1]!==0))i+=2;i+=2}else{while(i<body.length&&body[i]!==0)i++;i++}if(i<body.length){try{out.cover=await dataUrl(new Blob([body.slice(i)],{type:mime}))}catch{}}}pos+=10+size}}}catch(e){console.warn('LX ID3',e)}}
  if(/\.m4a$/i.test(file.name||''))try{Object.assign(out,await readMp4Tags(file))}catch(e){console.warn('LX M4A tags',e)}
  try{out.duration=await new Promise(resolve=>{const a=document.createElement('audio'),url=URL.createObjectURL(file);let timer,finished=false;const done=v=>{if(finished)return;finished=true;clearTimeout(timer);URL.revokeObjectURL(url);resolve(Number(v)||0)};a.preload='metadata';a.onloadedmetadata=()=>done(a.duration);a.onerror=()=>done(0);a.src=url;timer=setTimeout(()=>done(0),5000)})}catch{}
 ...
 async function applyEditor(meta,{force=false}={}){if(!meta)return;const q=id=>document.getElementById(id),set=(id,value,allowDefault=false)=>{const el=q(id);if(!el||value==null||value==='')return;if(force||!clean(el.value)||(allowDefault&&['Geral','Outros'].includes(clean(el.value))))el.value=String(value)};set('cTitle',meta.title);set('typeA',meta.artist);set('cYear',meta.year);set('cGenre',meta.genre,true);set('cDesc',meta.desc);if(LX.ui?.state)LX.ui.state.musicAutoMeta=meta;const info=q('selectedMediaInfo');if(info)info.textContent=`✓ Detectada: ${meta.artist||'Artista'} — ${meta.title||'Faixa'}${meta.album?' · '+meta.album:''}${meta.cover?' · capa automática':''}`;return meta}
 function fallbackFileMeta(file){const raw=stem(file?.name||'Música')||'Música',parts=partsFromTitle(raw),artist=parts.artist==='LX Music'?'':parts.artist,title=parts.title||raw;return {title,artist,album:'',genre:'Música',year:'',duration:0,cover:'',desc:artist?`${title} — ${artist}.`:`${title}.`,provider:'Nome do arquivo',confidence:1}}
 async function fillEditorFromFile(file,opts={}){const info=document.getElementById('selectedMediaInfo');if(info)info.textContent=`Lendo tags e buscando capa de ${file?.name||'áudio'}…`;const fallback=fallbackFileMeta(file);let meta=fallback;try{const detected=await probeFile(file);meta={...fallback,...detected,title:clean(detected?.title)||fallback.title,artist:clean(detected?.artist)&&detected.artist!=='LX Music'?detected.artist:fallback.artist,genre:clean(detected?.genre)||fallback.genre,desc:clean(detected?.desc)||fallback.desc}}catch(e){console.warn('LX music metadata fallback',e)}try{await applyEditor(meta,opts)}catch(e){console.warn('LX music editor fallback',e);const title=document.getElementById('cTitle'),artist=document.getElementById('typeA'),genre=document.getElementById('cGenre'),desc=document.getElementById('cDesc');if(title&&(opts.force||!clean(title.value)))title.value=meta.title;if(artist&&(opts.force||!clean(artist.value))&&meta.artist)artist.value=meta.artist;if(genre&&(opts.force||!clean(genre.value)||['Geral','Outros'].includes(clean(genre.value))))genre.value=meta.genre||'Música';if(desc&&(opts.force||!clean(desc.value)))desc.value=meta.desc||'';if(LX.ui?.state)LX.ui.state.musicAutoMeta=meta}if(info){const fromTags=meta.provider&&meta.provider!=='Nome do arquivo';info.textContent=fromTags?`✓ Nome e metadados automáticos: ${meta.artist?meta.artist+' — ':''}${meta.title}${meta.album?' · álbum: '+meta.album:''}${meta.cover?' · capa automática':''}`:`✓ Nome automático pelo arquivo: ${meta.artist?meta.artist+' — ':''}${meta.title}`;}return meta}
