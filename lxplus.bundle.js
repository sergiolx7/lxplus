...
(()=>{
 ...
 function describe(ref){
  ...
  if(/^https?:\/\//i.test(raw)){
   ...
   const yid=youtubeId(raw),ylist=youtubePlaylistId(raw);if(yid)return {kind:'embed',provider:'YouTube',label:'YouTube',src:`https://www.youtube.com/embed/${encodeURIComponent(yid)}?rel=0&playsinline=1&enablejsapi=1&origin=${encodeURIComponent(location.origin)}`,openUrl:`https://www.youtube.com/watch?v=${encodeURIComponent(yid)}`,embedHeight:152};
   if(ylist&&/(?:youtube\.com|youtu\.be|music\.youtube\.com)/i.test(raw))return {kind:'embed',provider:'YouTube',label:'YouTube Playlist',src:`https://www.youtube.com/embed/videoseries?list=${encodeURIComponent(ylist)}&rel=0&playsinline=1&enablejsapi=1&origin=${encodeURIComponent(location.origin)}`,openUrl:`https://www.youtube.com/playlist?list=${encodeURIComponent(ylist)}`,embedHeight:220};
  ...
  if(raw.startsWith('youtube-playlist:')){const id=raw.slice(17);return {kind:'embed',provider:'YouTube',label:'YouTube Playlist',src:`https://www.youtube.com/embed/videoseries?list=${encodeURIComponent(id)}&rel=0&playsinline=1&enablejsapi=1&origin=${encodeURIComponent(location.origin)}`,openUrl:`https://www.youtube.com/playlist?list=${encodeURIComponent(id)}`,embedHeight:220}};
  if(raw.startsWith('youtube:')){const id=raw.slice(8);return {kind:'embed',provider:'YouTube',label:'YouTube',src:`https://www.youtube.com/embed/${encodeURIComponent(id)}?rel=0&playsinline=1&enablejsapi=1&origin=${encodeURIComponent(location.origin)}`,openUrl:`https://www.youtube.com/watch?v=${encodeURIComponent(id)}`,embedHeight:152}};
...
function renderMusicExperience(){
 ...
 setTimeout(()=>LX.prewarmMusicCatalog?.(all),0);
 ...
 let content='';
 if(active==='home'){
  const mixes=categories.slice(0,5).map((g,i)=>musicMixCard(g,all.filter(x=>musicHasGenre(x,g)),i)).join('');
  const categoryCards=categories.map(g=>musicCategoryCard(g,all)).join('');
  const categoryShelves=categories.slice(0,4).map(g=>musicGridSection(g,`Seleção organizada em ${g}.`,all.filter(x=>musicHasGenre(x,g)).slice(0,12),`<button type="button" class="lx-text-btn" data-lx-music-genre="${esc(g)}">Ver tudo</button>`)).join('');
  content=`${now}${musicHomeHero(all,firstName)}<section class="lx-music-section lx-music-category-section"><div class="lx-section-title"><div><h2>Explorar por gênero</h2><p>Encontre suas músicas por categoria sem deixar a biblioteca bagunçada.</p></div><button type="button" class="lx-text-btn" data-lx-music-view="explore">Ver todas</button></div><div class="lx-music-category-grid">${categoryCards}</div></section><section class="lx-music-section lx-music-quick-section"><div class="lx-section-title"><div><h2>Ouça novamente</h2><p>Retome com um toque.</p></div></div><div class="lx-music-quick-grid-v48">${quick.map(musicQuickCard).join('')}</div></section>${mixes?`<section class="lx-music-section"><div class="lx-section-title"><div><h2>Seus mixes</h2><p>Seleções organizadas por estilo.</p></div></div><div class="lx-music-mixes">${mixes}</div></section>`:''}${musicGridSection('Feito para você','Recomendações baseadas na sua atividade e nos gêneros que você mais ouve.',recommended)}${musicGridSection('Popular agora','Os destaques da biblioteca LX Plus.',top)}${musicGridSection('Lançamentos','Os itens adicionados mais recentemente.',newest)}${categoryShelves}`;
 }else{
...
async function edit(id=null,initialType=null,prefill=null){
 ...
 const updateUpload=()=>{const t=$('cType').value,inp=$('typeMedia'),episodic=['Série','Anime','Dorama'].includes(t),music=t==='Música';$('quickUploadLabel').textContent=uploadLabel(t);inp.accept=acceptFor(t);inp.multiple=episodic;$('quickUploadHelp').textContent=(x?.mediaKey||x?.episodes?.length)?'Já existe mídia cadastrada. Envie somente se quiser substituir ou adicionar.':'Selecione o arquivo master na melhor qualidade disponível.';$('quickSeasonArea')?.classList.toggle('hidden',!episodic);$('cMusicGenresField')?.classList.toggle('hidden',!music);if($('cGenreLabel'))$('cGenreLabel').textContent=music?'Gênero principal':'Gênero';if(music&&$('cGenre')?.value==='Geral')$('cGenre').value='Outros';renderSeasonSummary();updateExternalFields()};
...
async function setupMusicEmbed(desc,autoplay=false){
 ...
 if(frame){
  ...
  if(kind==='youtube'){
   ...
   const join=src.includes('?')?'&':'?';if(!/enablejsapi=1/.test(src))src+=`${join}enablejsapi=1&origin=${encodeURIComponent(location.origin)}`;src+=`${src.includes('?')?'&':'?'}autoplay=${autoplay?1:0}`;
   ...
   frame.onload=()=>{clearTimeout(playTimer);attempts=0;providerPlayback.paused=!autoplay;setTimeout(requestPlay,160);updateMusicUI()};
  }
...
function syncMusicMediaSession(t=currentMusic()){
 ...
 try{
  ...
  if(isExternalMusicTrack(t)){navigator.mediaSession.playbackState='none';return}
...
function music(id,index=0,autoplay=true){
 ...
 if(autoplay&&cached&&a&&!isExternalMusicTrack(chosen)){
   try{
     ...
     Promise.resolve().then(async()=>{try{await waitForMusicReady(a,5000);await a.play();providerPlayback={kind:'native',paused:false,desc:null};updateMusicUI()}catch(error){console.warn('LX immediate music play',error);await recoverNativeMusicSource(chosen,true,error)}finally{rememberMusicStart(id,nextIndex)}});return
...
async function recoverNativeMusicSource(t,autoplay=true,previousError=null){
 const a=$('musicAudio'),ref=a?.dataset?.sourceRef||t?.mediaKey||'';if(!a||!ref)return false;
 try{if(String(ref).startsWith('cloud:')){const blob=await cloudBlobFallback(ref,t);if(blob){if(musicObjectUrl?.startsWith?.('blob:'))try{URL.revokeObjectURL(musicObjectUrl)}catch{};musicObjectUrl=URL.createObjectURL(blob);a.src=musicObjectUrl;a.load();await waitForMusicReady(a,10000);if(autoplay)await a.play();providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}}
 }catch(error){console.warn('LX native recovery failed',previousError,error)}
 setMusicStatus('Falha no áudio','error');LX.toast(previousError?.name==='NotAllowedError'?'O navegador bloqueou o início automático. Toque em ▶ novamente.':'Não consegui abrir esse áudio. Verifique o arquivo ou a conexão.');return false
}
async function loadTrack(autoplay=false){
 const t=currentMusic();if(!t)return false;const ticket=++musicLoadTicket,a=$('musicAudio'),dock=$('musicDock'),content=currentMusicContent()||{},trackMark=String(t.contentId)+'|'+String(t.index??state.musicIndex);
 if(autoplay)unlockMusicGesture();if(a.dataset.trackMark!==trackMark){a.dataset.trackMark=trackMark;delete a.dataset.recovery;delete a.dataset.blobRecovery}a.dataset.sourceRef='';setMusicStatus('Carregando faixa completa…','loading');
 ...
 const refs=[t.fullMediaKey,t.audioKey,t.mediaKey,t.audioUrl,t.url,t.src,content.fullMediaKey,content.audioKey,content.mediaKey].filter(Boolean);let src=null,embed=null,sourceRef=null;
 try{
  for(const ref of refs){const desc=LX.mediaSources?.describe?.(ref);if(desc?.kind==='embed'){embed=embed||desc;continue}let media=cachedMusicSource(ref);if(!media&&desc?.kind==='direct'&&desc.src)media=cacheMusicSource(ref,desc.src);if(!media)media=await prewarmMusicRef(ref);if(ticket!==musicLoadTicket)return false;if(media){sourceRef=ref;if(typeof media==='string')src=media;else{media=playableMusicBlob(media,t,ref);cacheMusicSource(ref,media);musicObjectUrl=URL.createObjectURL(media);src=musicObjectUrl}break}}
  if(!src&&embed){await setupMusicEmbed(embed,autoplay);if(ticket!==musicLoadTicket)return false;setMusicStatus('Player conectado','ready');updateMusicUI();syncMusicCardState();return true}
  if(!src){setMusicStatus('Áudio indisponível','error');if(autoplay)LX.toast('Não encontrei uma fonte reproduzível para esta faixa.');updateMusicUI();return false}
  a.dataset.sourceRef=String(sourceRef||'');a.preload='auto';a.src=String(src);a.load();
  try{await waitForMusicReady(a,8500)}catch(error){console.warn('LX audio ready failed',error);if(ticket!==musicLoadTicket)return false;const ok=await recoverNativeMusicSource(t,autoplay,error);updateMusicUI();return ok}
  if(ticket!==musicLoadTicket)return false;setMusicStatus('Faixa completa','ready');
  if(autoplay){try{await a.play();providerPlayback={kind:'native',paused:false,desc:null}}catch(error){console.warn('LX first audio play',error);const ok=await recoverNativeMusicSource(t,true,error);updateMusicUI();return ok}}
  updateMusicUI();return true
 }catch(error){console.warn('LX audio load',error);if(ticket!==musicLoadTicket)return false;const ok=await recoverNativeMusicSource(t,autoplay,error);updateMusicUI();return ok}
}
function updateMusicUI(){const a=$('musicAudio'),t=currentMusic(),paused=isExternalMusicTrack(t)?providerPlayback.paused:a.paused;$('musicPlay').innerHTML=LX.artwork.icon(paused?'play':'pause');$('musicPlay').title='Reproduzir / pausar';$('musicPlay').setAttribute('aria-label',$('musicPlay').title);$('musicShuffle').classList.toggle('active',musicShuffleMode);$('musicShuffle').setAttribute('aria-pressed',String(musicShuffleMode));$('musicRepeat').classList.toggle('active',musicRepeatMode>0);$('musicRepeat').innerHTML=LX.artwork.icon('repeat')+(musicRepeatMode===2?'<sup>1</sup>':'');$('musicRepeat').setAttribute('aria-pressed',String(musicRepeatMode>0));try{if('mediaSession'in navigator)navigator.mediaSession.playbackState=t?(isExternalMusicTrack(t)?'none':(paused?'paused':'playing')):'none'}catch{}syncMusicCardState();syncMusicSaveUI();syncRightMusicRail?.()}
...
document.addEventListener('visibilitychange',()=>{
 ...
 if(document.visibilityState==='hidden'){
  if(isYoutubeTrack(t)&&providerPlayback.paused===false){youtubeCommand('pauseVideo');providerPlayback.paused=true;lxYoutubeWasPlayingBeforeBackground=true;updateMusicUI()}
  else if(!isExternalMusicTrack(t)){syncMusicMediaSession(t);syncMusicMediaPosition($('musicAudio'),true)}
 }else{
  if(lxYoutubeWasPlayingBeforeBackground){lxYoutubeWasPlayingBeforeBackground=false;setMusicStatus('YouTube · toque em reproduzir para continuar','ready');updateMusicUI()}
  else if(!isExternalMusicTrack(t)){syncMusicMediaSession(t);syncMusicMediaPosition($('musicAudio'),true)}
 }
});
...
(()=>{
  ...
  function localRows(files){return [...(files||[])].filter(f=>/\.(?:mp4|mkv|webm|mov|m4v|avi|ts|mp3|m4a|aac|flac|wav|ogg|opus|pdf|epub|mobi|azw3?|cb[rz])$/i.test(f.name||'')).map(f=>({name:f.name,path:f.webkitRelativePath||f.name,url:'',size:f.size,type:'auto',source:'local-name'}))}
  ...
  async function analyzeRows(rows){
    ...
    try{for(let i=0;i<rows.length;i+=25){const chunk=rows.slice(i,i+25),d=await universalHub('catalog_match_batch',{items:chunk,type:'auto'});for(const x of d.items||[]){const dup=duplicateKind(x);x.finalStatus=dup?.kind||x.status;x.existing=dup?.item||null;x.selected=x.finalStatus==='matched'||x.finalStatus==='merge';state.queue.push(x)}renderQueue();setBatchStatus(`Metadados automáticos: ${Math.min(i+25,rows.length)}/${rows.length}`,Math.min(i+25,rows.length),rows.length)}toast('Análise concluída. Revise apenas os itens marcados em amarelo.')}catch(e){console.warn(e);toast(errorText(e));setBatchStatus(`Falha: ${errorText(e)}`,0,rows.length,'error')}
  }
  ...
  function createFromMatch(row,published){const x=metadataToCatalog(row.match,published),key=mediaKey(row.url);if(row.parsed.type==='movie'&&key){x.mediaKey=key;x.qualityMode=String(key).startsWith('gdrive:')?'drive-auto':'external';x.sourceProvider=String(key).startsWith('gdrive:')?'gdrive':'direct'}else if(row.parsed.type==='music'&&key){x.mediaKey=key;x.tracks=[{number:1,title:x.title,artist:x.artist,duration:x.duration||0,mediaKey:key,qualityMode:'external'}]}else if(row.parsed.type==='book'&&row.url){x.externalReadUrl=row.url;x.mediaKey=key||row.url}return x}
  ...
  async function publishBatch(){const btn=document.getElementById('universalPublish'),published=!!document.getElementById('universalPublishNow')?.checked,items=buildPublishItems(published);if(!items.length)return toast('Selecione pelo menos um item identificado.');const prev=btn?.textContent;if(btn){btn.disabled=true;btn.textContent='Publicando lote…'}try{let saved=0;for(let i=0;i<items.length;i+=400){const part=items.slice(i,i+400);if(LX.cloud?.bulkUpsertCatalogItems)saved+=await LX.cloud.bulkUpsertCatalogItems(part);else{for(const x of part){await D().saveCatalogItem(x);saved++}}}toast(`${saved} título${saved===1?'':'s'} salvo${saved===1?'':'s'} no catálogo.`);setBatchStatus(`Concluído: ${saved} títulos salvos`,saved,saved,'ok');LX.ui?.renderApp?.()}catch(e){console.warn(e);toast(`Falha ao publicar: ${errorText(e)}`)}finally{if(btn){btn.disabled=false;btn.textContent=prev||'Publicar selecionados'}}}
