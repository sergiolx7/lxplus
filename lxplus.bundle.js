...
function music(id,index=0,autoplay=true){
 ...
 const nextIndex=Math.max(0,Math.min(index,tracks.length-1)),same=String(currentMusic()?.contentId)===String(id)&&currentMusic()?.index===nextIndex&&!$('musicDock')?.classList.contains('hidden');if(same){if(autoplay)toggleCurrentMusic();return}
 ...
 const queueItems=[x,...peers];
 D.track('music',{id,title:x.title});state.musicQueue=queueItems.flatMap(item=>{
  const list=item.tracks?.length?item.tracks:((item.mediaKey||item.authorizedAudioUrl||item.externalMusicUrl)?[{number:1,title:item.title,mediaKey:item.mediaKey||item.authorizedAudioUrl||item.externalMusicUrl}]:[]);
