...
async function play(id,epNum=1,epSeason=null){
 ...
 const openDrivePreview=()=>{LX.toast('A reprodução permanece dentro da LX Plus. Para compatibilidade máxima, migre este arquivo para R2/LX Stream.');};
 ...
 v.onerror=async()=>{loading.classList.add('hide');const keep=Number.isFinite(v.currentTime)?v.currentTime:0,currentInfo=LX.mediaSources?.describe?.(key)||sourceInfo;if(currentInfo.provider==='Google Drive'){const candidates=Array.isArray(currentInfo.streamCandidates)?currentInfo.streamCandidates.filter(Boolean):[],idx=Number(mediaCandidateIndex.get(String(key))||0);if(idx<candidates.length-1){mediaCandidateIndex.set(String(key),idx+1);errorText.textContent='Tentando outra rota segura do Google Drive dentro da LX Plus…';error.classList.remove('hide');await setSource(keep);return}errorText.textContent='O Google Drive recusou o streaming direto deste arquivo. A LX Plus não vai abrir tela de download nem tirar você do site. Para reprodução estável em todos os aparelhos, envie o vídeo para R2/LX Stream em MP4 H.264/AAC ou HLS.';error.classList.remove('hide');v.controls=false;nativeMode=false;root.classList.remove('native');return}if(sourceRefreshes<2){errorText.textContent='Renovando a fonte segura do vídeo…';error.classList.remove('hide');await setSource(keep);return}errorText.textContent='O arquivo não pôde ser reproduzido neste navegador. Tente outra fonte compatível com MP4/HLS.';error.classList.remove('hide');v.controls=true;nativeMode=true;root.classList.add('native')};
...
(()=>{
  ...
  function enhancePlayer(host){
    ...
    const style=document.createElement('style');style.textContent=`.heading strong,.copy b{font-size:clamp(20px,2.1vw,32px)!important;line-height:1.08!important}.heading small,.copy small{font-size:clamp(12px,1vw,15px)!important}.nextCard strong{font-size:19px!important}.nextCard small{font-size:14px!important;line-height:1.45!important}.stage.lx-image-enhanced video{filter:contrast(1.085) saturate(1.045) brightness(1.018);transform:translateZ(0);image-rendering:auto}.settings button.active{background:rgba(255,255,255,.14)!important;color:#fff!important}.lx-source-quality{display:block;padding:8px 10px;margin-bottom:4px;border-radius:9px;background:rgba(255,255,255,.055);color:#c8d0d8;font-size:11px}.nextCard{padding:18px!important}@media(max-width:760px){.heading strong,.copy b{font-size:17px!important}.heading small,.copy small{font-size:12px!important}.nextCard{bottom:76px!important}.nextCard strong{font-size:17px!important}.nextCard small{font-size:13px!important}}`;
    ...
    if(!root.getElementById('enhanceV27')){const button=document.createElement('button');button.id='enhanceV27';button.textContent='Melhoria inteligente · desligada';button.setAttribute('aria-pressed','false');button.onclick=()=>{const on=stage.classList.toggle('lx-image-enhanced');button.textContent=`Melhoria inteligente · ${on?'ligada':'desligada'}`;button.setAttribute('aria-pressed',String(on));toast(on?'Nitidez e contraste aprimorados no aparelho. Isso melhora a percepção, mas não cria 4K real a partir de baixa resolução.':'Melhoria de imagem desligada.')};settings.prepend(button)}
...
(()=>{
 ...
 window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['music-smart-metadata']='31.6';
})();
