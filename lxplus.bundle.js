...
/* ===== lxplus.js ===== */
window.__LX_JS_BUILD='NOVA-20260924-R2-AUDIO';

/* ===== config.js · LX Plus v25.50 ===== */
window.LX=window.LX||{};
LX.config={
  version:'NOVA-20260924-R2-AUDIO',
...
function cacheMusicSource(ref,value){if(!ref||!value)return value;musicSourceCache.set(String(ref),{value,until:Date.now()+20*60*1000});return value}
...
async function fetchMusicTicket(ref,expires=7200){
 ...
 // Prefer the controlled streaming route. It exposes CORS and byte-range headers required by
 // HTMLAudioElement + WebAudio effects, then falls back to a normal signed Storage URL.
 try{
  ...
  if(access){
   ...
   if(!error&&(data?.streamUrl||data?.url||data?.signedUrl)){
    ...
    musicSourceFailures.delete(key);return url
   }
...
function prewarmMusicCatalog(list=[]){return Promise.resolve([])}
function prewarmMusicContent(id){return Promise.resolve([])}
...
async function recoverNativeMusicSource(t,autoplay=true,previousError=null,quiet=false,expectedTicket=musicLoadTicket){
 ...
 const trySource=async(media)=>{
  ...
  try{
   ...
   a.dataset.sourceRef=String(ref);a.crossOrigin='anonymous';a.preload='metadata';a.src=String(source);a.load();await waitForMusicReady(a,10000);
   if(!current())return false;
   if(autoplay)await musicDeadline(a.play(),10000,'PLAY_TIMEOUT');
   if(!current())return false;
...
async function loadTrack(autoplay=false){
 ...
 musicLoadGuard=setTimeout(()=>{if(ticket!==musicLoadTicket)return;++musicLoadTicket;a.pause();a.removeAttribute('src');a.load();delete a.dataset.loadingTrackTicket;setMusicStatus('Tempo esgotado · toque em ▶ para tentar novamente','error');updateMusicUI()},30000);
 ...
 try{
  for(const ref of [...new Set(refs)]){
   ...
   try{
    ...
    a.dataset.sourceRef=String(ref);a.crossOrigin='anonymous';a.preload='metadata';a.src=String(source);a.load();await waitForMusicReady(a,10000);if(ticket!==musicLoadTicket)return false;
    if(autoplay)await musicDeadline(a.play(),10000,'PLAY_TIMEOUT');providerPlayback={kind:'native',paused:a.paused,desc:null};setMusicStatus(a.paused?'Pronta · toque em ▶':'Reproduzindo','ready');updateMusicUI();return true
