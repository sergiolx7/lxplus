...
function renderMusicExperience(){
 ...
 let content='';
 if(active==='home'){
  const featuredOnly=musicHighlights(all),hero=musicHomeHero(featuredOnly,firstName);
  content=`${now}${hero||`<section class="lx-music-top lx-music-home-empty"><div class="lx-music-greeting"><span class="eyebrow">LX MUSIC</span><h1>${greeting()}, ${esc(firstName)}</h1><p>Nenhuma música foi marcada como destaque ainda.</p></div><button type="button" class="primary-btn" data-lx-music-view="explore">Pesquisar músicas</button></section>`}<section class="lx-music-home-search"><div><span class="eyebrow">CATÁLOGO COMPLETO</span><h2>Quer ouvir outra música?</h2><p>As demais faixas ficam fora da página principal e aparecem na pesquisa.</p></div><button type="button" class="primary-btn" data-lx-music-view="explore">Pesquisar no catálogo</button></section>`;
 }else{
...
async function edit(id=null,initialType=null,prefill=null){
 ...
 $('cExternalMedia')?.addEventListener('blur',()=>{if($('cType')?.value==='Música'&&mediaSource==='youtube'&&$('cExternalMedia')?.value?.trim())LX.musicMeta?.fillEditorFromYoutube?.($('cExternalMedia').value.trim(),{force:!x}).catch(()=>{})});
...
(()=>{
 ...
 async function matchRemote(meta,fileName='track.mp3'){const c=LX.cloud?.db?.();if(!c)return null;const name=`${clean(meta?.artist)?clean(meta.artist)+' - ':''}${clean(meta?.title)||stem(fileName)||'Música'}.mp3`;try{const {data,error}=await c.functions.invoke('lx-universal-importer',{body:{action:'catalog_match_batch',items:[{name,type:'music'}]}});if(error||data?.error)return null;const row=data?.items?.[0];return row?.match?{...row.match,confidence:Number(row.confidence||0)}:null}catch(e){console.warn('LX music metadata match',e);return null}}
 ...
 async function probeYoutube(value){const id=youtubeId(value);if(!id)throw new Error('YOUTUBE_LINK_INVALID');let yt={title:'',artist:'',cover:`https://i.ytimg.com/vi/${id}/hqdefault.jpg`,duration:0,externalUrl:`https://www.youtube.com/watch?v=${id}`,remoteId:id,provider:'YouTube'};try{const r=await fetch(`https://www.youtube.com/oembed?url=${encodeURIComponent(yt.externalUrl)}&format=json`,{mode:'cors',credentials:'omit'});if(r.ok){const d=await r.json(),p=partsFromTitle(d.title,d.author_name);yt={...yt,title:p.title,artist:p.artist,cover:d.thumbnail_url||yt.cover}}else throw new Error('OEMBED_'+r.status)}catch{try{const c=LX.cloud?.db?.();if(c){const {data,error}=await c.functions.invoke('lx-content-hub',{body:{action:'youtube_bulk',source:yt.externalUrl,maxItems:1}});if(!error&&data?.items?.[0]){const row=data.items[0],p=partsFromTitle(row.title,row.channelTitle);yt={...yt,title:p.title,artist:p.artist,cover:row.thumbnail||yt.cover,duration:Number(row.duration||0)}}}}catch{}}
  const remote=await matchRemote(yt,`${yt.artist||'YouTube'} - ${yt.title||id}.mp3`);return merge({},remote,yt)
 }
 ...
 LX.musicMeta={readId3,probeFile,probeYoutube,applyEditor,fillEditorFromFile,fillEditorFromYoutube,youtubeId,partsFromTitle,merge};
 window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['music-smart-metadata']='31.2';
})();
