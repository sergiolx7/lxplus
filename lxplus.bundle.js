...
/* ===== lxplus.js ===== */
window.__LX_JS_BUILD='NOVA-20260924-R11-DRIVE-FIX';

/* ===== config.js · LX Plus v25.50 ===== */
window.LX=window.LX||{};
LX.config={
  version:'NOVA-20260924-R11-DRIVE-FIX',
...
(()=>{
 ...
 async function test(ep=endpoint()){
  ...
  return {ok:true,endpoint:ep,googleConfigured:data.googleConfigured===true,mode:data.mode||'public-drive',service:data.service||'LX Storage',version:data.version||'legacy'};
 }
 ...
 LX.driveStorage={endpoint,configured,streamUrl,probeUrl,test,saveEndpoint,deployUrl,openAutoDeploy,workerRepo:WORKER_REPO};
})();
window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['drive-storage']='R11';
...
(()=>{
 ...
 const driveStreamCandidates=(id,resourceKey='')=>{id=clean(id);resourceKey=clean(resourceKey);if(!id)return[];const q=encodeURIComponent(id),rk=resourceKey?`&resourcekey=${encodeURIComponent(resourceKey)}`:'',lx=LX.driveStorage?.streamUrl?.(id,resourceKey)||'';return [lx,`https://drive.usercontent.google.com/download?id=${q}&export=download&confirm=t${rk}`,`https://drive.google.com/uc?export=download&id=${q}&confirm=t${rk}`].filter(Boolean)};
 ...
 function describe(ref){
  ...
  if(/^https?:\/\//i.test(raw)){
   ...
   const dr=driveRef(raw),did=dr.id;if(did&&/drive\.google\.com/i.test(raw)){const previewSrc=`https://drive.google.com/file/d/${encodeURIComponent(did)}/preview${dr.resourceKey?`?resourcekey=${encodeURIComponent(dr.resourceKey)}`:''}`,streamCandidates=driveStreamCandidates(did,dr.resourceKey);return {kind:'direct',provider:'LX Storage · Drive',label:'LX Storage · Google Drive',src:streamCandidates[0]||previewSrc,streamCandidates,previewSrc,driveId:did,resourceKey:dr.resourceKey}};
  ...
  if(raw.startsWith('gdrive:')){const dr=driveRef(raw),id=dr.id,previewSrc=`https://drive.google.com/file/d/${encodeURIComponent(id)}/preview${dr.resourceKey?`?resourcekey=${encodeURIComponent(dr.resourceKey)}`:''}`,streamCandidates=driveStreamCandidates(id,dr.resourceKey);return {kind:'direct',provider:'LX Storage · Drive',label:'LX Storage · Google Drive',src:streamCandidates[0]||previewSrc,streamCandidates,previewSrc,driveId:id,resourceKey:dr.resourceKey}};
 ...
 const info={
  ...
  gdrive:{label:'LX Storage · Google Drive',placeholder:'https://drive.google.com/file/d/.../view',help:'Cole somente o link normal do arquivo no Google Drive. O endpoint LX Storage já é aplicado automaticamente e o vídeo abre no LX Player. No modo por link, deixe o arquivo como Qualquer pessoa com o link · Leitor.'},
...
async function edit(id=null,initialType=null,prefill=null){
 ...
 <form id="contentForm" class="quick-content-form">
  <div class="quick-core">
   ...
   <div id="quickDriveReady" class="quick-drive-ready span2 ${['Filme','Série','Anime','Dorama'].includes(type)?'':'hidden'}"><div><span class="quick-drive-dot"></span><strong>LX Storage conectado</strong><small>Google Drive → LX Storage Worker → LX Player. Para publicar, basta colar o link do Drive.</small></div><span id="quickDriveState" class="quick-drive-state">${LX.driveStorage?.configured?.()?'ATIVO':'CONFIGURAR'}</span></div>
...
async function play(id,epNum=1,epSeason=null){
 ...
 if(!!sourceInfo.driveId&&!LX.driveStorage?.configured?.()&&!settings.querySelector('[data-drive-compat]')){const b=document.createElement('button');b.dataset.driveCompat='1';b.textContent='Google Drive · modo compatibilidade';b.onclick=()=>{settings.classList.add('hide');activateDriveFallback('manual')};settings.prepend(b)}
