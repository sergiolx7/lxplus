...
async function prewarmMusicRef(ref){
 ...
 const task=(async()=>{
  ...
  if(key.startsWith('cloud:')){
   ...
   if(LX.cloud?.db?.()){try{const path=key.slice(6).replace(/^\/+/, '');const signed=await LX.cloud.db().storage.from('lx-media').createSignedUrl(path,7200);if(!signed.error&&signed.data?.signedUrl){musicSignedFallbackCache.set(key,signed.data.signedUrl);return cacheMusicSource(key,signed.data.signedUrl)}}catch(e){console.warn('LX signed music fallback',key,e)}}
  }
...
function prewarmMusicCatalog(list=[]){const refs=[];for(const x of list.slice(0,80))for(const ref of musicRefsForContent(x))if(!refs.includes(ref))refs.push(ref);return Promise.allSettled(refs.slice(0,120).map(ref=>prewarmMusicRef(ref)))}
...
function music(id,index=0,autoplay=true){
 ...
 const chosen=state.musicQueue[nextIndex],ref=chosen?.mediaKey||'',cached=ref?cachedMusicSource(ref):null,a=$('musicAudio');setMusicDockArtwork(chosen,x);$('musicTitle').textContent=chosen.title||'Faixa';$('musicArtist').textContent=chosen.artist||'LX Music';
...
async function recoverNativeMusicSource(t,autoplay=true,previousError=null){
 ...
 try{if(String(ref).startsWith('cloud:')){musicSourceCache.delete(String(ref));const ticket=await fetchMusicTicket(String(ref),7200);if(ticket){try{cacheMusicSource(ref,ticket);a.src=ticket;a.load();await waitForMusicReady(a,12000);if(autoplay)await a.play();providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}catch(e){console.warn('LX stream ticket recovery',e)}}const storedSigned=musicSignedFallbackCache.get(String(ref));if(storedSigned){try{a.src=storedSigned;a.load();await waitForMusicReady(a,12000);if(autoplay)await a.play();cacheMusicSource(ref,storedSigned);providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}catch(e){console.warn('LX stored signed recovery',e)}}if(LX.cloud?.db?.()){try{const path=String(ref).slice(6).replace(/^\/+/, '');const signed=await LX.cloud.db().storage.from('lx-media').createSignedUrl(path,7200);if(!signed.error&&signed.data?.signedUrl){musicSignedFallbackCache.set(String(ref),signed.data.signedUrl);cacheMusicSource(ref,signed.data.signedUrl);a.src=signed.data.signedUrl;a.load();await waitForMusicReady(a,12000);if(autoplay)await a.play();providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}}catch(e){console.warn('LX client signed fallback',e)}}const blob=await cloudBlobFallback(ref,t);if(blob){if(musicObjectUrl?.startsWith?.('blob:'))try{URL.revokeObjectURL(musicObjectUrl)}catch{};musicObjectUrl=URL.createObjectURL(blob);a.src=musicObjectUrl;a.load();await waitForMusicReady(a,10000);if(autoplay)await a.play();providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}}
