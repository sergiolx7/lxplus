...
/* ===== lxplus.js ===== */
window.__LX_JS_BUILD='NOVA-20260923';

/* ===== config.js · LX Plus v25.50 ===== */
window.LX=window.LX||{};
LX.config={
  version:'NOVA-20260923',
...
async function fetchMusicTicket(ref,expires=7200){
 ...
 try{
  ...
  if(access){
   ...
   if(!error&&(data?.signedUrl||data?.url)){const url=data.signedUrl||data.url;musicSourceFailures.delete(key);musicSignedFallbackCache.set(key,url);return url}
  ...
  else musicSourceFailures.set(key,new Error('Sessão expirada ao solicitar áudio'));
 }catch(error){musicSourceFailures.set(key,error);console.warn('LX media ticket fallback failed',error)}
 return null
}
...
async function recoverNativeMusicSource(t,autoplay=true,previousError=null,quiet=false,expectedTicket=musicLoadTicket){
 ...
 const trySource=async(media)=>{
  ...
  try{
   ...
   if(!current())return false;
   if(autoplay)await musicDeadline(a.play(),6500,'PLAY_TIMEOUT');
   if(!current())return false;
 ...
 try{
  for(const ref of [...new Set(refs)]){
   ...
   try{
    ...
    if(autoplay)await musicDeadline(a.play(),6500,'PLAY_TIMEOUT');providerPlayback={kind:'native',paused:a.paused,desc:null};setMusicStatus(a.paused?'Pronta · toque em ▶':'Reproduzindo','ready');updateMusicUI();return true
