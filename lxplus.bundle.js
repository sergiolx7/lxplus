...
(()=>{
  ...
  function candidates(item={},content={}){const yt=ytId(item.sourceMediaKey||item.mediaKey||item.url||content.sourceMediaKey||content.mediaKey||content.url||content.tracks?.[0]?.mediaKey),ytArt=valid(item.youtubeThumbnail||content.youtubeThumbnail||(yt?`https://i.ytimg.com/vi/${yt}/hqdefault.jpg`:'')),spotify=valid(item.spotifyArtwork||item.images?.[0]?.url||item.album?.images?.[0]?.url||content.spotifyArtwork||content.album?.images?.[0]?.url),isYoutube=!!yt;const preferred=isYoutube?[ytArt,item.cover,item.artwork?.url,item.albumCover,item.album?.cover,content.cover,content.image,content.carouselImage,content.banner,spotify,fallback]:[item.cover,item.artwork?.url,item.albumCover,item.album?.cover,content.cover,content.image,content.carouselImage,content.banner,spotify,ytArt,fallback];return [...new Set(preferred.map(valid).filter(Boolean))]}
...
async function edit(id=null,initialType=null,prefill=null){
 ...
 <form id="contentForm" class="quick-content-form">
  ...
  <details class="advanced-content span2" id="advancedContent"><summary>Mais opções <small>capa, gênero, classificação, destaque...</small></summary><div class="form-grid advanced-content-grid">
   ...
   <label class="field span2">Créditos <span class="optional">opcional</span><input id="typeA" value="${esc(x?.director||x?.creator||x?.studio||x?.author||x?.artist||'')}" placeholder="Diretor, criador, estúdio, autor ou artista"></label><label id="authorizedAudioField" class="field span2 ${type==='Música'?'':'hidden'}"><span>Áudio autorizado para segundo plano <em class="optional">opcional</em></span><input id="cAuthorizedAudio" type="url" inputmode="url" autocomplete="off" value="${esc(x?.authorizedAudioUrl||x?.authorizedStreamUrl||'')}" placeholder="https://.../audio.mp3 ou endpoint autorizado"><small>Quando preenchido, a LX Plus usa esta fonte no player nativo e mantém apenas o selo LX Music + provedor. Ideal para tocar com tela bloqueada/segundo plano.</small></label>
  </div></details>
 ...
 const updateUpload=()=>{const t=$('cType').value,inp=$('typeMedia'),episodic=['Série','Anime','Dorama'].includes(t),music=t==='Música';$('quickUploadLabel').textContent=uploadLabel(t);inp.accept=acceptFor(t);inp.multiple=episodic;$('quickUploadHelp').textContent=(x?.mediaKey||x?.episodes?.length)?'Já existe mídia cadastrada. Envie somente se quiser substituir ou adicionar.':'Selecione o arquivo master na melhor qualidade disponível.';$('quickSeasonArea')?.classList.toggle('hidden',!episodic);$('cMusicGenresField')?.classList.toggle('hidden',!music);$('authorizedAudioField')?.classList.toggle('hidden',!music);if($('cGenreLabel'))$('cGenreLabel').textContent=music?'Gênero principal':'Gênero';if(music&&$('cGenre')?.value==='Geral')$('cGenre').value='Outros';renderSeasonSummary();updateExternalFields()};
 ...
 $('contentForm').onsubmit=async e=>{
  ...
  try{
   ...
   if(y.type==='Filme')y.director=credit||y.director||'';if(['Série','Dorama'].includes(y.type))y.creator=credit||y.creator||'';if(y.type==='Anime')y.studio=credit||y.studio||'';if(y.type==='Livro')y.author=credit||y.author||'';if(y.type==='Música'){y.artist=credit||y.artist||'';const authUrl=$('cAuthorizedAudio')?.value?.trim()||'';if(authUrl){if(!/^https:\/\//i.test(authUrl))throw new Error('AUTHORIZED_AUDIO_HTTPS_REQUIRED');y.authorizedAudioUrl=authUrl}else delete y.authorizedAudioUrl}
...
async function prewarmMusicRef(ref){
 ...
 const task=(async()=>{const d=LX.mediaSources?.describe?.(key);if(d?.kind==='embed')return null;if(d?.kind==='direct'&&d.src)return cacheMusicSource(key,d.src);if(key.startsWith('cloud:')&&LX.cloud?.db?.()){try{const {data,error}=await LX.cloud.db().functions.invoke('lx-media-ticket',{body:{key,expires:7200}});if(!error&&data?.url)return cacheMusicSource(key,data.url)}catch(e){console.warn('LX media ticket',key,e)}try{const path=key.slice(6).replace(/^\/+/, '');const signed=await LX.cloud.db().storage.from('lx-media').createSignedUrl(path,7200);if(!signed.error&&signed.data?.signedUrl)return cacheMusicSource(key,signed.data.signedUrl)}catch(e){console.warn('LX signed music fallback',key,e)}}try{const value=await S.getMedia(key);return value?cacheMusicSource(key,value):null}catch(e){console.warn('LX prewarm music source',key,e);return null}})().finally(()=>musicSourcePromises.delete(key));musicSourcePromises.set(key,task);return task
}
function prewarmMusicCatalog(list=[]){const refs=[];for(const x of list.slice(0,24)){const tracks=x.tracks?.length?x.tracks:[x];for(const t of tracks.slice(0,3)){const ref=t.authorizedAudioUrl||t.authorizedStreamUrl||t.authorizedAudioKey||t.fullMediaKey||t.audioKey||t.audioUrl||t.mediaKey||t.url||x.authorizedAudioUrl||x.authorizedStreamUrl||x.authorizedAudioKey||x.fullMediaKey||x.audioKey||x.audioUrl||x.mediaKey;if(ref&&!refs.includes(ref))refs.push(ref)}}refs.slice(0,28).forEach(ref=>prewarmMusicRef(ref).catch(()=>{}))}
...
function setMusicDockArtwork(item=currentMusic(),content=currentMusicContent?.()||{}){if(!item)return;const stable={...item,cover:item.cover||content.cover||item.youtubeThumbnail||content.youtubeThumbnail||LX.artwork.fallback};LX.artwork.set($('musicCover'),stable,content);$('musicDock')?.style.setProperty('--music-cover',LX.artwork.safeBackground(stable,content))}
...
function syncMusicMediaSession(t=currentMusic()){
 ...
 try{
  ...
  if(isExternalMusicTrack(t)){
   ...
   navigator.mediaSession.setActionHandler?.('play',()=>{if(providerPlayback.kind==='spotify'){try{spotifyController?.play?.();providerPlayback.paused=false}catch{}}else if(providerPlayback.kind==='youtube'){youtubeCommand('playVideo');providerPlayback.paused=false}updateMusicUI()});
   navigator.mediaSession.setActionHandler?.('pause',()=>{if(providerPlayback.kind==='spotify'){try{spotifyController?.pause?.();providerPlayback.paused=true}catch{}}else if(providerPlayback.kind==='youtube'){youtubeCommand('pauseVideo');providerPlayback.paused=true}updateMusicUI()});
...
function music(id,index=0,autoplay=true){
 ...
 if(autoplay&&cached&&a&&!isExternalMusicTrack(chosen)){
   try{
     ...
     $('musicTitle').textContent=chosen.title||'Faixa';$('musicArtist').textContent=chosen.artist||'LX Music';syncMusicProviderBrand(chosen);setMusicDockArtwork(chosen,content);setMusicStatus('Faixa completa','ready');syncMusicMediaSession(chosen);updateMusicUI();
...
async function recoverNativeMusicSource(t,autoplay=true,previousError=null){
 ...
 try{if(String(ref).startsWith('cloud:')){musicSourceCache.delete(String(ref));if(LX.cloud?.db?.()){try{const {data,error}=await LX.cloud.db().functions.invoke('lx-media-ticket',{body:{key:String(ref),expires:7200}});if(!error&&data?.url){cacheMusicSource(ref,data.url);a.src=data.url;a.load();await waitForMusicReady(a,10000);if(autoplay)await a.play();providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}}catch(e){console.warn('LX media ticket recovery',e)}try{const path=String(ref).slice(6).replace(/^\/+/, '');const signed=await LX.cloud.db().storage.from('lx-media').createSignedUrl(path,7200);if(!signed.error&&signed.data?.signedUrl){cacheMusicSource(ref,signed.data.signedUrl);a.src=signed.data.signedUrl;a.load();await waitForMusicReady(a,10000);if(autoplay)await a.play();providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}}catch(e){console.warn('LX client signed fallback',e)}}const blob=await cloudBlobFallback(ref,t);if(blob){if(musicObjectUrl?.startsWith?.('blob:'))try{URL.revokeObjectURL(musicObjectUrl)}catch{};musicObjectUrl=URL.createObjectURL(blob);a.src=musicObjectUrl;a.load();await waitForMusicReady(a,10000);if(autoplay)await a.play();providerPlayback={kind:'native',paused:!autoplay,desc:null};setMusicStatus('Faixa completa','ready');return true}}
 }catch(error){console.warn('LX native recovery failed',previousError,error)}
 setMusicStatus('Falha no áudio','error');LX.toast(previousError?.name==='NotAllowedError'?'O navegador bloqueou o início automático. Toque em ▶ novamente.':'Não consegui abrir esta faixa. A LX Plus tentou a fonte principal e os fallbacks seguros.');return false
}
async function loadTrack(autoplay=false){
 ...
 $('musicTitle').textContent=t.title||'Faixa';$('musicArtist').textContent=t.artist||'LX Music';syncMusicProviderBrand(t);setMusicDockArtwork(t,content);$('musicProgress').value=0;$('musicTime').textContent='0:00';$('musicDuration').textContent=LX.fmt(t.duration||0);if($('musicProviderLabel'))$('musicProviderLabel').textContent='LX Music';syncMusicMediaSession(t);syncMusicCardState();document.dispatchEvent(new CustomEvent('lx:music-changed',{detail:{item:t,index:state.musicIndex,provider:musicProvider(content)}}));
 ...
 try{
  ...
  if(!src&&embed){await setupMusicEmbed(embed,autoplay);if(ticket!==musicLoadTicket)return false;setMusicStatus(embed?.provider==='Spotify'?'Spotify conectado':'YouTube conectado','ready');syncMusicProviderBrand(t);updateMusicUI();syncMusicCardState();return true}
...
(()=>{
 ...
 window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['music-smart-metadata']='32.1';
})();


/* ===== LX Plus v32.1 · resilient media helpers ===== */
(()=>{
 'use strict';
 ...
 window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['v32.1-resilient-audio']='32.1';
})();
