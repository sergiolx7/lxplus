...
/* ===== lxplus.js ===== */
window.__LX_JS_BUILD='R12-NEXUS-20260924';

/* ===== config.js · LX Plus v25.50 ===== */
window.LX=window.LX||{};
LX.config={
  version:'R12-NEXUS-20260924',
...
/* ===== lx-drive-storage.js · LX Plus R9 =====
...
(()=>{
 ...
 async function test(ep=endpoint()){
  ...
  const r=await fetch(`${ep}/health?check=1`,{cache:'no-store'});let data={};try{data=await r.json()}catch{}
  if(!r.ok||data.ok===false)throw new Error(data.error||`LX_STORAGE_HTTP_${r.status}`);
  return {ok:true,endpoint:ep,googleConfigured:data.googleConfigured===true,googleConnected:data.googleConnected===true,mode:data.mode||'unconfigured',error:data.error||'',service:data.service||'LX Storage',version:data.version||'legacy'};
 }
 ...
 function explain(code){return ({DRIVE_AUTH_NOT_CONFIGURED:'Configure as credenciais do Google Drive no Worker.',DRIVE_AUTH_INVALID:'O Google recusou a autenticação. Confira as credenciais do Worker.',DRIVE_PERMISSION_OR_QUOTA:'O Google recusou o arquivo: confira o acesso da conta de serviço e a cota do Drive.',FILE_NOT_FOUND_OR_NO_ACCESS:'Arquivo não encontrado ou sem permissão para a conta de serviço.',DRIVE_DOWNLOAD_DISABLED:'O proprietário bloqueou o download deste arquivo.',DRIVE_RANGE_UNAVAILABLE:'O Drive não respondeu com bytes parciais (206).',DRIVE_RETURNED_NON_MEDIA:'O Google respondeu com uma página ou JSON em vez de vídeo.',NOT_A_MEDIA_FILE:'O link aponta para um documento/atalho, não um arquivo de vídeo.',DRIVE_FORMAT_INCOMPATIBLE:'Formato incompatível com muitos navegadores. Use MP4 H.264 + AAC.'})[String(code||'')]||'Falha ao consultar o LX Storage. Confira a conexão e tente novamente.'}
 async function probe(value){const {id,resourceKey}=ref(value);if(!/^[A-Za-z0-9_-]{10,}$/.test(id))throw new Error('GDRIVE_LINK_INVALID');const url=probeUrl(id,resourceKey);if(!url)throw new Error('DRIVE_AUTH_NOT_CONFIGURED');const r=await fetch(url,{cache:'no-store'});let data={};try{data=await r.json()}catch{}if(!r.ok||!data.ok)throw new Error(data.error||'LX_STORAGE_HTTP_'+r.status);return data}
 ...
 LX.driveStorage={endpoint,configured,streamUrl,probeUrl,probe,ref,explain,test,saveEndpoint,deployUrl,openAutoDeploy,workerRepo:WORKER_REPO};
})();
...
function bind(page){
 ...
 if(page==='uploads'){
  const refreshDriveStorage=async()=>{const badge=$('driveStorageStatusBadge'),help=$('driveStorageHelp'),inp=$('driveStorageEndpoint');if(inp&&!inp.value)inp.value=LX.driveStorage?.endpoint?.()||'';if(!badge)return;const ep=(inp?.value||LX.driveStorage?.endpoint?.()||'').trim();if(!ep){badge.textContent='Endpoint não configurado';badge.className='r2-status warn';if(help)help.textContent='Implante o Worker, configure as credenciais no painel Cloudflare e salve seu endpoint.';return}badge.textContent='Testando…';badge.className='r2-status warn';try{const st=await LX.driveStorage.test(ep);badge.textContent=st.googleConnected?'● Online · Google Drive conectado':st.googleConfigured?'● Worker online · autenticação com erro':'● Worker online · Google Drive não configurado';badge.className='r2-status '+(st.googleConnected?'ok':'warn');if(help)help.textContent=st.googleConnected?'Drive API autenticada. Cole o link do arquivo e teste antes de publicar. Espaço pessoal não disponível via conta de serviço.':'Conclua a conexão do Google no Worker. Diagnóstico: '+(st.error||'credenciais ausentes')}catch(e){badge.textContent='● LX Storage offline';badge.className='r2-status warn';if(help)help.textContent='Confira o endereço do Worker e tente novamente.'}};
