...
(()=>{
  ...
  const logo='assets/lxplus-logo-v27.png?v=27.0';
  const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms));
  const standalone=()=>window.matchMedia?.('(display-mode: standalone)').matches||navigator.standalone===true;

  LX.v27={version:'27.0',spotifyCache:{tracks:[],artists:[],albums:[],playlists:[]}};
  ...
  function spotifyCards(items,type){
    ...
    return (items||[]).map(item=>`<button type="button" class="lx-v27-spotify-card" onclick="LX.${method}('${esc(item.id)}')"><img src="${esc(imageOf(item))}" alt="" loading="lazy"><span><strong>${esc(item.name)}</strong><small>${type==='artist'?'Artista':type==='album'?esc(artistsOf(item)):'Playlist oficial'}</small></span></button>`).join('');
  }
  ...
  function syncNowPlaying(){
    ...
    const preview=isPreview(item),spotify=isSpotify(item),queue=state().musicQueue||[],current=Number(state().musicIndex)||0;
    const signature=[item.mediaKey||item.remoteId||item.contentId||item.title,current,queue.map(row=>row.mediaKey||row.remoteId||row.contentId||row.title).join(',')].join('|');
    if(panel.dataset.signature===signature){updateNowPlayingProgress();return}panel.dataset.signature=signature;
    panel.style.setProperty('--now-art',`url("${String(item.cover||'').replace(/"/g,'%22')}")`);
    panel.innerHTML=`<div class="lx-v27-now-backdrop"></div><header><button type="button" onclick="LX.closeNowPlaying()" aria-label="Fechar">⌄</button><div><span>TOCANDO AGORA</span><strong>${preview?'Prévia oficial':spotify?'Spotify oficial':'LX Music'}</strong></div><button type="button" onclick="LX.openMusicQueue()" aria-label="Abrir fila">☷</button></header><main><section class="lx-v27-now-art"><img src="${esc(item.cover||'')}" alt="Capa de ${esc(item.title||'música')}"><span class="${preview?'preview':''}">${preview?'PRÉVIA · TRECHO CURTO':spotify?'PLAYER OFICIAL SPOTIFY':'FAIXA COMPLETA'}</span></section><section class="lx-v27-now-copy"><div><span>${esc(item.album||'LX Music')}</span><h2>${esc(item.title||'Faixa')}</h2><p>${esc(item.artist||'LX Music')}</p></div><button type="button" onclick="document.getElementById('musicLikeBtn')?.click()" aria-label="Salvar">♡</button><div class="lx-v27-now-progress"><input data-now-progress type="range" min="0" max="100" step="0.1" value="${Number($('musicProgress')?.value||0)}" oninput="document.getElementById('musicProgress').value=this.value;document.getElementById('musicProgress').dispatchEvent(new Event('input'))"><div><span data-now-time>${esc($('musicTime')?.textContent||'0:00')}</span><span data-now-duration>${esc($('musicDuration')?.textContent||fmt(item.duration))}</span></div></div><div class="lx-v27-now-controls"><button type="button" onclick="document.getElementById('musicShuffle')?.click()">⇄</button><button type="button" onclick="document.getElementById('musicPrev')?.click()">‹</button><button data-now-play type="button" class="main" onclick="document.getElementById('musicPlay')?.click()">${audio()?.paused?'▶':'❚❚'}</button><button type="button" onclick="document.getElementById('musicNext')?.click()">›</button><button type="button" onclick="document.getElementById('musicRepeat')?.click()">↻</button></div><div class="lx-v27-now-actions"><button type="button" onclick="LX.openMusicLyrics?.()">≡ Letra</button><button type="button" onclick="LX.openMusicQueue()">☷ Fila · ${queue.length}</button>${spotify?`<a href="https://open.spotify.com/track/${esc(item.remoteId||String(item.mediaKey).split(':').pop())}" target="_blank" rel="noopener">Abrir no Spotify ↗</a>`:''}</div>${preview?'<div class="lx-v27-preview-warning"><strong>Isto é uma prévia.</strong><span>O trecho pode começar no meio da música. Para ouvir a faixa completa, abra o serviço oficial.</span></div>':''}</section><aside><span>PRÓXIMAS</span><h3>Na fila</h3>${queue.slice(current+1,current+5).map((row,index)=>`<button type="button" onclick="LX.musicQueuePlay(${current+1+index})"><img src="${esc(row.cover||'')}" alt=""><span><strong>${esc(row.title)}</strong><small>${esc(row.artist)}</small></span></button>`).join('')||'<p>Fim da fila.</p>'}</aside></main>`;
  }
  function updateNowPlayingProgress(){const panel=$('lxV27NowPlaying'),source=$('musicProgress');if(!panel||panel.classList.contains('hidden'))return;const range=panel.querySelector('[data-now-progress]'),time=panel.querySelector('[data-now-time]'),duration=panel.querySelector('[data-now-duration]'),play=panel.querySelector('[data-now-play]');if(range&&source&&document.activeElement!==range)range.value=source.value||0;if(time)time.textContent=$('musicTime')?.textContent||'0:00';if(duration)duration.textContent=$('musicDuration')?.textContent||fmt(track()?.duration);if(play)play.textContent=audio()?.paused?'▶':'❚❚'}
  ...
  window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES.v27='27.0';
})();
