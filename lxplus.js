...
window.__LX_MODULES['free-media-hub']='29.7';
...
function musicProvider(){return 'LX Music'}
...
function renderMusicExperience(){
 ...
 setTimeout(()=>prewarmMusicCatalog(all),0);
 ...
 if(active==='explore'&&state.musicGenre!=='Todos')visible=visible.filter(x=>x.genre===state.musicGenre);
 ...
 let content='';
 if(active==='home'){
  const mixes=genres.slice(0,5).map((g,i)=>musicMixCard(g,all.filter(x=>x.genre===g),i)).join('');
  content=`${now}${musicHomeHero(all,firstName)}<section class="lx-music-section lx-music-quick-section"><div class="lx-section-title"><div><h2>Ouça novamente</h2><p>Retome com um toque.</p></div></div><div class="lx-music-quick-grid-v48">${quick.map(musicQuickCard).join('')}</div></section>${mixes?`<section class="lx-music-section"><div class="lx-section-title"><div><h2>Seus mixes</h2><p>Seleções organizadas por estilo.</p></div></div><div class="lx-music-mixes">${mixes}</div></section>`:''}${musicGridSection('Feito para você','Recomendações baseadas na sua atividade.',recommended)}${musicGridSection('Popular agora','Os destaques da biblioteca LX Plus.',top)}${musicGridSection('Lançamentos','Os itens adicionados mais recentemente.',newest)}`;
 }else{
  const title=active==='liked'?'Músicas curtidas':active==='recent'?'Tocadas recentemente':active==='library'?'Sua biblioteca':state.query?`Resultados para “${state.query}”`:'Buscar músicas';
  const subtitle=active==='liked'?'Tudo o que você salvou em um só lugar.':active==='recent'?'Continue ouvindo de onde parou.':active==='library'?'Seus salvos e reproduções recentes.':'Pesquise por música, artista, álbum ou gênero.';
  const genreButtons=['Todos',...genres].map(g=>`<button type="button" data-lx-music-genre="${esc(g)}" class="${state.musicGenre===g?'active':''}">${esc(g)}</button>`).join('');
  content=`${now}<section class="lx-music-browser-head"><div class="lx-music-browser-copy"><span class="eyebrow">LX MUSIC · ${active==='explore'?'DESCOBRIR':'COLEÇÃO'}</span><h1>${esc(title)}</h1><p>${esc(subtitle)}</p></div><form id="lxMusicSearchForm" class="lx-music-search-box"><span>⌕</span><input id="lxMusicSearchInput" value="${esc(state.query)}" placeholder="O que você quer ouvir?" aria-label="Buscar músicas"><button type="button" id="lxMusicSearchClear" class="${state.query?'':'hidden'}" aria-label="Limpar busca">×</button><button type="submit">Buscar</button></form></section>${active==='explore'?`<section class="lx-music-browser-tools"><div class="lx-music-filter-chips">${genreButtons}</div><label>Ordenar<select id="lxMusicSort"><option value="recent" ${state.musicSort==='recent'?'selected':''}>Mais recentes</option><option value="title" ${state.musicSort==='title'?'selected':''}>Título</option><option value="artist" ${state.musicSort==='artist'?'selected':''}>Artista</option></select></label></section>`:''}${visible.length?`<section class="lx-music-section lx-music-results"><div class="lx-section-title"><div><h2>${visible.length} ${visible.length===1?'resultado':'resultados'}</h2><p>Toque na capa para reproduzir ou no título para abrir o álbum.</p></div>${active!=='explore'?'<button type="button" class="lx-text-btn" data-lx-music-view="explore">Explorar</button>':''}</div><div class="lx-music-grid">${visible.map(musicAlbumCard).join('')}</div></section>`:musicEmpty(active==='liked'?'Nenhuma música curtida':active==='recent'?'Nada reproduzido ainda':state.query?'Nenhum resultado':'Sua biblioteca está vazia',active==='liked'?'Use o coração nos álbuns para salvá-los aqui.':active==='recent'?'Reproduza uma música e ela aparecerá nesta área.':'Tente outro título, artista ou gênero.','<button type="button" class="primary-btn" data-lx-music-view="explore">Explorar músicas</button>')}`;
 }
 $('homeContent').innerHTML=`<div class="lx-music-app-shell lx-music-shell-v260">${side}<main class="lx-music-main lx-music-main-spotify lx-music-main-v260">${mobile}${content}</main></div>`;bindMusicExperience();
}
LX.musicSetView=musicSetView;LX.musicSetGenre=musicSetGenre;
...
function render(page='dashboard'){U.state.adminPage=page;const m=$('adminMain');({dashboard,library,importer,uploads,requests,community,admins,premium,analytics,notifications,appearance,settings}[page]||dashboard)(m);bind(page);updateSaveDock();LX.saveView?.()}
...
function bind(page){
 ...
 if(page==='uploads'){
  ...
  $('processBatch').onclick=async()=>{const id=+$('batchContent')?.value,a=D.catalog(),x=a.find(z=>z.id===id);if(!x||!files.length)return LX.toast('Escolha uma obra e os arquivos.');const btn=$('processBatch');btn.disabled=true;try{if(U.state.uploadMode==='tracks'){
    x.tracks=x.tracks||[];let n=+$('batchStart')?.value||x.tracks.length+1;for(const f of files){const key=await S.putMedia('track_'+x.id+'_'+Date.now()+'_'+f.name,f);x.tracks.push({number:n++,title:f.name.replace(/\.[^.]+$/,''),duration:0,mediaKey:key,fileName:f.name||'',mimeType:f.type||'',size:Number(f.size||0),qualityMode:'lx-auto'});LX.primeMusicMedia?.(key,f)}x.tracks.sort((a,b)=>(a.number||0)-(b.number||0));
   }else{
...
async function edit(id=null,initialType=null,prefill=null){
 ...
 <form id="contentForm" class="quick-content-form">
  <div class="quick-core">
   ...
   <section id="driveQualityField" class="drive-quality-field span2 hidden">
    <div class="drive-quality-head"><div><span class="eyebrow">QUALIDADE · GOOGLE DRIVE</span><strong>Versões opcionais por qualidade</strong><small>O link principal continua em Auto. Se você subir versões separadas no Drive, a pessoa poderá escolher 4K, 1080p, 720p ou 480p no player.</small></div><span class="drive-quality-auto">AUTO</span></div>
 ...
 const updateUpload=()=>{const t=$('cType').value,inp=$('typeMedia'),episodic=['Série','Anime','Dorama'].includes(t);$('quickUploadLabel').textContent=uploadLabel(t);inp.accept=acceptFor(t);inp.multiple=episodic;$('quickUploadHelp').textContent=(x?.mediaKey||x?.episodes?.length)?'Já existe mídia cadastrada. Envie somente se quiser substituir ou adicionar.':'Selecione o arquivo master na melhor qualidade disponível.';$('quickSeasonArea')?.classList.toggle('hidden',!episodic);renderSeasonSummary();updateExternalFields()};
 ...
 $('testExternalMedia').onclick=()=>{try{const d=LX.mediaSources.preview(mediaSource,$('cExternalMedia').value);$('externalMediaDetected').textContent=`✓ ${d.provider} reconhecido · pronto para salvar`;if(d.openUrl)window.open(d.openUrl,'_blank','noopener')}catch(err){$('externalMediaDetected').textContent='';LX.toast(err.message||'Não reconheci esse link.')}};
 ...
 updateUpload();$('cType').onchange=()=>{updateUpload();setMediaSource(mediaSource)};$('typeMedia').onchange=()=>{const files=[...($('typeMedia').files||[])],el=$('selectedMediaInfo');if(!el)return;el.textContent=files.length?files.map(f=>`${f.name} · ${(f.size/1024/1024).toFixed(1)} MB`).join(' | '):''};
 ...
 $('contentForm').onsubmit=async e=>{
  ...
  try{
   ...
   if(coverFile)y.cover=await dataUrl(coverFile);if(!y.cover)y.cover=defaultArtwork(y.title,y.type,false);if(bannerFile)y.banner=await dataUrl(bannerFile);if(carouselFile)y.carouselImage=await dataUrl(carouselFile);if(['Filme','Série','Anime','Dorama'].includes(y.type)&&(!y.banner||LX.isAutoArtwork?.(y.banner)))y.banner=y.cover||defaultArtwork(y.title,y.type,true);if(['Livro','Música'].includes(y.type)){delete y.banner;delete y.carouselImage;}
   let expectedMediaKey=null,expectedEpisodes=[],expectedQualities=[];if(mediaSource!=='upload'){
    ...
    if(['Série','Anime','Dorama'].includes(y.type)){
     ...
     const entered=$('cExternalMedia')?.value?.trim()||'';if(entered){const externalKey=LX.mediaSources.normalize(mediaSource,entered);y.mediaKey=externalKey;expectedMediaKey=externalKey;y.autoQuality={status:'external',source:mediaSource,updatedAt:new Date().toISOString()};if(y.type==='Música')y.tracks=[{number:1,title:y.title,duration:0,mediaKey:externalKey,qualityMode:'external'}]}
     if(mediaSource==='gdrive'&&['Filme'].includes(y.type)){y.qualitySources={...(y.qualitySources||{})};for(const q of DRIVE_QUALITY_KEYS){const val=$(driveQualityInputId(q))?.value?.trim()||'';if(val){y.qualitySources[q]=LX.mediaSources.normalize('gdrive',val);expectedQualities.push({scope:'main',q,mediaKey:y.qualitySources[q]})}else delete y.qualitySources[q]}if(!Object.keys(y.qualitySources).length)delete y.qualitySources;else y.qualityMode='drive-multi'}
    }
   }
    ...
    else{const f=files[0],key=await S.putMedia(`main_${y.id}_${Date.now()}_${f.name}`,f);uploadedNow.push(key);y.mediaKey=key;y.autoQuality={status:'ready',source:String(key).startsWith('r2:')?'r2':'upload',updatedAt:new Date().toISOString()};if(y.type==='Música'){y.tracks=[{number:1,title:y.title,duration:0,mediaKey:key,qualityMode:'lx-auto',fileName:f.name||'',mimeType:f.type||'',size:Number(f.size||0)}];LX.primeMusicMedia?.(key,f)}}
   }
...
LX.admin={render,edit,del,verify,saveChanges,discardPending,approveUser,rejectUser,deletePendingUser,makeAdmin,removeAdmin,reqStatus,premiumSet,premiumOff,togglePublish,toggleFeatured,priority,preview,fromRequest,pendingCount,updateSaveDock};updateSaveDock();})();
...
function syncRightMusicRail(){
 const t=currentMusic?.(),a=$('musicAudio'),sp=t&&isSpotifyTrack?.(t),external=$('musicDock')?.classList.contains('external-provider'),paused=t?(sp?spotifyPlayback.paused:(a?.paused!==false)):true;
 ...
 if(playBtn)playBtn.textContent=external&&!sp?'♫':(paused?'▶':'❚❚');
 let pos=0,total=0;if(sp){pos=(spotifyPlayback.position||0)/1000;total=(spotifyPlayback.duration||0)/1000}else if(a){pos=+a.currentTime||0;total=+a.duration||+t?.duration||0}
...
async function play(id,epNum=1,epSeason=null){
 ...
 const resolveMedia=async ref=>{if(!ref)return null;const info=LX.mediaSources?.describe?.(ref);if(info?.kind==='direct')return info.src;if(/^https?:\/\//i.test(String(ref)))return ref;const b=await S.getMedia(ref);return b?(typeof b==='string'?b:URL.createObjectURL(b)):null};
 ...
 const openDrivePreview=()=>{if(!sourceInfo.previewSrc)return;try{v.pause();v.removeAttribute('src');v.load()}catch{};let fr=q('#drivePreviewFrame');if(!fr){fr=document.createElement('iframe');fr.id='drivePreviewFrame';fr.className='driveFrame';fr.title=`${x.title} · Google Drive`;fr.allow='autoplay; fullscreen; picture-in-picture; encrypted-media';fr.allowFullscreen=true;fr.referrerPolicy='strict-origin-when-cross-origin';stage.appendChild(fr)}fr.src=sourceInfo.previewSrc;root.classList.add('drive-fallback');loading.classList.add('hide');error.classList.add('hide');LX.toast('Usando o player de compatibilidade do Google Drive.')};
 ...
 v.onerror=async()=>{loading.classList.add('hide');const keep=Number.isFinite(v.currentTime)?v.currentTime:0;if(sourceInfo.provider==='Google Drive'){errorText.textContent='O Stream Direto do Drive não abriu neste formato. Para assistir sem esperar o processamento do Google, use MP4 com vídeo H.264 e áudio AAC. Você também pode abrir o player do Drive como compatibilidade.';error.classList.remove('hide');v.controls=true;nativeMode=true;root.classList.add('native');return}if(sourceRefreshes<2){errorText.textContent='Renovando a fonte segura do vídeo…';error.classList.remove('hide');await setSource(keep);return}errorText.textContent='O arquivo não pôde ser reproduzido neste navegador. Você pode tentar os controles do navegador.';error.classList.remove('hide');v.controls=true;nativeMode=true;root.classList.add('native')};
...
async function prewarmMusicRef(ref){
 ...
 const task=(async()=>{const d=LX.mediaSources?.describe?.(key);if(d?.kind==='embed')return null;if(d?.kind==='direct'&&d.src)return cacheMusicSource(key,d.src);try{const value=await S.getMedia(key);return value?cacheMusicSource(key,value):null}catch(e){console.warn('LX prewarm music source',key,e);return null}})().finally(()=>musicSourcePromises.delete(key));musicSourcePromises.set(key,task);return task
}
function prewarmMusicCatalog(list=[]){const refs=[];for(const x of list.slice(0,24)){const tracks=x.tracks?.length?x.tracks:[x];for(const t of tracks.slice(0,3)){const ref=t.fullMediaKey||t.audioKey||t.mediaKey||t.audioUrl||t.url||x.fullMediaKey||x.audioKey||x.mediaKey;if(ref&&!refs.includes(ref))refs.push(ref)}}refs.slice(0,28).forEach(ref=>prewarmMusicRef(ref).catch(()=>{}))}
...
function musicEmbedDesc(item=currentMusic(),content=currentMusicContent?.()||{}){if(!item)return null;const refs=[item.fullMediaKey,item.audioKey,item.mediaKey,item.audioUrl,item.url,item.src,content.fullMediaKey,content.audioKey,content.mediaKey].filter(Boolean);for(const ref of refs){const d=LX.mediaSources?.describe?.(ref);if(d?.kind==='embed')return d}return null}
function isSpotifyTrack(item=currentMusic()){const d=musicEmbedDesc(item);return !!d&&(d.provider==='Spotify'||!!d.spotifyUri)}
function isYoutubeTrack(item=currentMusic()){const d=musicEmbedDesc(item);return !!d&&/^YouTube/i.test(String(d.provider||''))}
function isExternalMusicTrack(item=currentMusic()){return !!musicEmbedDesc(item)}
function musicProvider(){return 'LX Music'}
...
async function ensureSpotifyController(uri,autoplay=false){const host=$('spotifyEmbedHost');if(!host)throw new Error('SPOTIFY_HOST_MISSING');spotifyAutoplayPending=!!autoplay;host.classList.remove('hidden');$('musicProviderFrame')?.classList.add('hidden');const load=ctrl=>{spotifyPlayback={paused:true,buffering:false,position:0,duration:0,uri};try{ctrl.loadEntity?.(uri)}catch{try{ctrl.loadUri?.(uri)}catch{}}if(autoplay){setTimeout(()=>{try{ctrl.play?.()}catch{}},80);setTimeout(()=>{if(spotifyPlayback.paused){$('musicDock')?.classList.add('provider-open');LX.toast('Se o navegador bloquear o início automático, toque em ▶ no player do Spotify ou no botão central da LX Plus.')}},950)}updateMusicUI();syncMusicCardState();return ctrl};if(spotifyController)return load(spotifyController);if(spotifyControllerPromise){const c=await spotifyControllerPromise;return load(c)}spotifyControllerPromise=(async()=>{const api=await waitSpotifyAPI();return await new Promise((resolve,reject)=>{try{api.createController(host,{width:'100%',height:152,uri},ctrl=>{spotifyController=ctrl;LX.spotifyEmbed.controller=ctrl;ctrl.addListener?.('ready',()=>{if(spotifyAutoplayPending){spotifyAutoplayPending=false;try{ctrl.play?.()}catch{}}});ctrl.addListener?.('playback_started',e=>{spotifyPlayback.paused=false;providerPlayback.paused=false;spotifyPlayback.uri=e?.data?.playingURI||spotifyPlayback.uri;updateMusicUI();syncMusicCardState()});ctrl.addListener?.('playback_update',e=>{providerPlayback.paused=!!e?.data?.isPaused;updateSpotifyProgress(e?.data||{})});resolve(ctrl)})}catch(e){reject(e)}})})();try{return await spotifyControllerPromise}finally{spotifyControllerPromise=null}}
...
function openMusicAlbum(id){const x=D.catalog().find(i=>String(i.id)===String(id));if(!x)return;const tracks=(x.tracks?.length?x.tracks:(x.mediaKey?[{title:x.title,mediaKey:x.mediaKey,duration:0}]:[])),provider=musicProvider(x),totalSecs=tracks.reduce((a,t)=>a+(+t.duration||0),0),saved=D.myList().some(z=>String(z)===String(id));$('modal').innerHTML=`<button class="close-btn" onclick="LX.ui.close()">×</button><div class="lx-album-page lx-album-page-v256 lx-album-page-v260"><section class="lx-album-hero"><div class="lx-album-cover" style="background-image:url('${x.cover||x.banner||''}')"></div><div class="lx-album-copy"><span class="eyebrow">LX MUSIC · ${tracks.length>1?'ÁLBUM':'SINGLE'}</span><h2>${esc(x.title)}</h2><p>${esc(x.artist||'LX Music')} · ${x.year||'2026'} · ${esc(x.genre||'Música')}</p><div class="lx-album-badges"><span>${esc(provider)}</span><span>${tracks.length} ${tracks.length===1?'faixa':'faixas'}</span><span>${totalSecs?LX.fmt(totalSecs):'Ao vivo'}</span></div><div class="hero-actions"><button class="primary-btn" onclick="LX.music(${id},0)">▶ Reproduzir</button><button class="secondary-btn" onclick="LX.musicShuffleAlbum(${id})">⇄ Embaralhar</button><button id="lxAlbumSave" class="secondary-btn ${saved?'active':''}" onclick="LX.musicToggleSaved(${id})">${saved?'♥ Salvo':'♡ Salvar'}</button>${x.lyrics||x.desc?'<button class="secondary-btn" onclick="LX.openMusicLyrics('+id+',0)">≡ Letra</button>':''}</div></div></section><div class="lx-track-list lx-track-list-v256"><div class="lx-track-list-head"><span>#</span><span>Título</span><span>Duração</span><span></span></div>${tracks.map((t,i)=>`<button class="lx-track-row" data-music-id="${id}" onclick="LX.music(${id},${i})"><b>${i+1}</b><span><strong>${esc(t.title||`Faixa ${i+1}`)}</strong><small>${esc(t.artist||x.artist||'LX Music')}</small></span><em>${LX.fmt(t.duration||0)}</em><i data-music-play-icon>▶</i></button>`).join('')||'<div class="notice">Nenhuma faixa enviada ainda.</div>'}</div></div>`;$('overlay').classList.remove('hidden');queueMicrotask(syncMusicCardState)}
async function toggleCurrentMusic(){const t=currentMusic();if(!t)return;if(isExternalMusicTrack(t)){const d=providerPlayback.desc||musicEmbedDesc(t);if(!d){loadTrack(true);return}if(providerPlayback.kind==='spotify'){try{providerPlayback.paused?spotifyController?.play?.():spotifyController?.pause?.();providerPlayback.paused=!providerPlayback.paused}catch{loadTrack(true)}updateMusicUI();return}if(providerPlayback.kind==='youtube'){youtubeCommand(providerPlayback.paused?'playVideo':'pauseVideo');providerPlayback.paused=!providerPlayback.paused;updateMusicUI();return}openMusicProvider();return}const a=$('musicAudio');if(!a)return;if(!a.src||a.error||a.networkState===3){await loadTrack(false);if(!a.src||a.error){LX.toast('Ainda não consegui preparar o áudio. Aguarde um instante e toque novamente.');return}}if(!a.paused){a.pause();return}try{await a.play();setMusicStatus('Faixa completa','ready')}catch(error){console.warn('LX manual play',error);const ref=a.dataset.sourceRef||t.mediaKey||'';if(ref&&String(ref).startsWith('cloud:')){const blob=await cloudBlobFallback(ref,t);if(blob){try{if(musicObjectUrl?.startsWith?.('blob:'))URL.revokeObjectURL(musicObjectUrl);musicObjectUrl=URL.createObjectURL(blob);a.src=musicObjectUrl;a.load();await a.play();setMusicStatus('Faixa completa','ready');updateMusicUI();return}catch(e){error=e}}}setMusicStatus('Falha no áudio','error');LX.toast(error?.name==='NotAllowedError'?'Toque novamente em ▶ para liberar o som.':'Não consegui iniciar esse áudio. O check-up marcou falha de reprodução.') }updateMusicUI()}
...
function music(id,index=0,autoplay=true){
 ...
 D.track('music',{id,title:x.title});state.musicQueue=tracks.map((track,i)=>({...track,mediaKey:track.fullMediaKey||track.audioKey||track.mediaKey||track.audioUrl||track.url||x.fullMediaKey||x.audioKey||x.mediaKey||'',contentId:id,index:i,title:String(track.title||x.title||'Faixa').trim()||'Faixa',artist:String(track.artist||x.artist||'LX Music').trim()||'LX Music',cover:LX.artwork.url(track,x),album:x.title,albumCover:x.cover||'',contentCover:x.cover||''}));state.musicIndex=nextIndex;$('musicDock').classList.remove('hidden');
 const chosen=state.musicQueue[nextIndex],ref=chosen?.mediaKey||'',cached=ref?cachedMusicSource(ref):null,a=$('musicAudio');
 // v29.7: if prewarming already resolved the file, start playback inside the click gesture.
 if(autoplay&&cached&&a&&!isExternalMusicTrack(chosen)){
   try{
     const content=x;let src=cached;
     a.pause();a.removeAttribute('src');if(musicObjectUrl?.startsWith?.('blob:'))try{URL.revokeObjectURL(musicObjectUrl)}catch{}musicObjectUrl=null;resetMusicProvider();
     if(src instanceof Blob){src=playableMusicBlob(src,chosen,ref);musicObjectUrl=URL.createObjectURL(src);src=musicObjectUrl}
     a.dataset.trackMark=String(id)+'|'+String(nextIndex);a.dataset.sourceRef=String(ref);a.preload='auto';a.src=String(src);a.load();
     $('musicTitle').textContent=chosen.title||'Faixa';$('musicArtist').textContent=chosen.artist||'LX Music';LX.artwork.set($('musicCover'),chosen,content);$('musicDock')?.style.setProperty('--music-cover',LX.artwork.safeBackground(chosen,content));setMusicStatus('Faixa completa','ready');syncMusicMediaSession(chosen);updateMusicUI();
     const started=a.play();Promise.resolve(started).then(()=>{providerPlayback={kind:'native',paused:false,desc:null};updateMusicUI();rememberMusicStart(id,nextIndex)}).catch(error=>{console.warn('LX immediate music play',error);loadTrack(false).finally(()=>rememberMusicStart(id,nextIndex))});return
   }catch(e){console.warn('LX immediate source path',e)}
 }
 loadTrack(autoplay).finally(()=>rememberMusicStart(id,nextIndex))
}
let musicLoadTicket=0,musicReadyPromise=Promise.resolve(false);
function setMusicStatus(text='',kind=''){const el=$('musicPlaybackKind');if(!el)return;el.textContent=text||'Faixa completa';el.classList.remove('hidden','is-loading','is-error','is-ready');if(kind)el.classList.add('is-'+kind)}
...
async function loadTrack(autoplay=false){
 const t=currentMusic();if(!t)return;const ticket=++musicLoadTicket,a=$('musicAudio'),dock=$('musicDock'),content=currentMusicContent()||{},trackMark=String(t.contentId)+'|'+String(t.index??state.musicIndex);
 if(a.dataset.trackMark!==trackMark){a.dataset.trackMark=trackMark;delete a.dataset.recovery}a.dataset.sourceRef='';setMusicStatus('Carregando faixa completa…','loading')
 ...
 const refs=[t.fullMediaKey,t.audioKey,t.mediaKey,t.audioUrl,t.url,t.src,content.fullMediaKey,content.audioKey,content.mediaKey].filter(Boolean);
 let src=null,embed=null,sourceRef=null,playError=null;
 try{
   for(const ref of refs){
     const desc=LX.mediaSources?.describe?.(ref);if(desc?.kind==='embed'){embed=embed||desc;continue}
     let media=cachedMusicSource(ref);
     if(!media&&desc?.kind==='direct'&&desc.src)media=cacheMusicSource(ref,desc.src);
     if(!media){try{media=await prewarmMusicRef(ref)}catch(e){console.warn('LX music source failed',ref,e)}}
     if(ticket!==musicLoadTicket)return;
     if(media){sourceRef=ref;if(typeof media==='string')src=media;else{media=playableMusicBlob(media,t,ref);cacheMusicSource(ref,media);src=URL.createObjectURL(media)}break}
   }
   ...
   if(!src){setMusicStatus('Áudio indisponível','error');if(autoplay)LX.toast('O arquivo existe no catálogo, mas o navegador não conseguiu abrir a mídia. Tente novamente.');updateMusicUI();return}
   musicObjectUrl=src.startsWith?.('blob:')?src:null;a.dataset.sourceRef=String(sourceRef||'');a.preload='auto';a.src=src;a.load();setMusicStatus('Faixa completa','ready');
   if(autoplay){try{await a.play();providerPlayback={kind:'native',paused:false,desc:null}}catch(error){playError=error;console.warn('LX first audio play',error)}}
   if(ticket!==musicLoadTicket)return;
   if(playError&&typeof src==='string'&&!src.startsWith('blob:')&&sourceRef&&playError?.name!=='NotAllowedError'){
     try{
       const ctrl=new AbortController(),tm=setTimeout(()=>ctrl.abort(),12000),res=await fetch(src,{signal:ctrl.signal,cache:'no-store'});clearTimeout(tm);
       if(!res.ok)throw new Error('AUDIO_FETCH_'+res.status);let blob=await res.blob();
       blob=playableMusicBlob(blob,t,sourceRef);
       cacheMusicSource(sourceRef,blob);if(musicObjectUrl?.startsWith?.('blob:'))try{URL.revokeObjectURL(musicObjectUrl)}catch{};musicObjectUrl=URL.createObjectURL(blob);a.src=musicObjectUrl;a.load();playError=null;if(autoplay)await a.play();providerPlayback={kind:'native',paused:false,desc:null};
     }catch(error){playError=error;console.warn('LX blob audio fallback',error)}
   }
   if(playError&&sourceRef&&String(sourceRef).startsWith('cloud:')&&playError?.name!=='NotAllowedError'){
     const blob=await cloudBlobFallback(sourceRef,t);if(blob&&ticket===musicLoadTicket){try{if(musicObjectUrl?.startsWith?.('blob:'))URL.revokeObjectURL(musicObjectUrl);musicObjectUrl=URL.createObjectURL(blob);a.src=musicObjectUrl;a.load();playError=null;if(autoplay)await a.play();providerPlayback={kind:'native',paused:false,desc:null};setMusicStatus('Faixa completa','ready')}catch(error){playError=error}}
   }
   if(playError){setMusicStatus(playError?.name==='NotAllowedError'?'Pronta para tocar':'Falha no áudio',playError?.name==='NotAllowedError'?'ready':'error');if(playError?.name==='NotAllowedError')LX.toast('A faixa está pronta. Toque em ▶ novamente para liberar o áudio neste navegador.');else LX.toast('O player não conseguiu decodificar ou abrir esse áudio. Use MP3/AAC/M4A/WAV compatível.')}
 }catch(error){console.warn('LX audio load',error);setMusicStatus('Falha ao carregar','error');if(ticket===musicLoadTicket&&autoplay)LX.toast('Não foi possível carregar o áudio completo desta faixa.')}
 if(ticket===musicLoadTicket)updateMusicUI();return !playError
}
...
window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['app']='29.7';
