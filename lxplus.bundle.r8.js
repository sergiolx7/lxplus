...
(()=>{
 ...
 async function applyEditor(meta,{force=false}={}){if(!meta)return null;const q=id=>document.getElementById(id),set=(id,value,allowDefault=false)=>{const el=q(id);if(!el||value==null||value==='')return;if(force||!clean(el.value)||(allowDefault&&['Geral','Outros','Música'].includes(clean(el.value))))el.value=String(value)};set('cTitle',meta.title);set('typeA',meta.artist);set('cYear',meta.year);set('cGenre',meta.genre,true);set('cDesc',meta.desc);LX.ui=LX.ui||{};LX.ui.state=LX.ui.state||{};LX.ui.state.musicAutoMeta=meta;const info=q('selectedMediaInfo');if(info)info.textContent=`✓ R8 automático: ${meta.artist||'Artista'} — ${meta.title||'Faixa'}${meta.album?' · '+meta.album:''}${meta.cover?' · capa automática':''}`;return meta}
 async function fillEditorFromFile(file,opts={}){const info=document.getElementById('selectedMediaInfo');if(info)info.textContent=`R8 analisando MP3: ${file?.name||'arquivo'}…`;try{const meta=await probeFile(file);return await applyEditor(meta,opts)}catch(e){console.warn('LX MP3 metadata primary fallback',e);const base=partsFromTitle(stem(file?.name||'Música.mp3'));const meta={title:base.title||stem(file?.name||'Música')||'Música',artist:base.artist&&base.artist!=='LX Music'?base.artist:'LX Music',album:'',year:'',genre:'Música',cover:'',duration:0,desc:`${base.title||'Música'} — ${base.artist||'LX Music'}.`,provider:'Nome do arquivo MP3'};try{return await applyEditor(meta,opts)}catch(inner){console.warn('LX MP3 metadata final fallback',inner);if(info)info.textContent=`✓ R8 MP3 selecionado: ${meta.artist} — ${meta.title}`;return meta}}}
...
/* ===== LX Plus NOVA R8 · MP3 automatic metadata hardening ===== */
(()=>{
 'use strict';
 const LX=window.LX=window.LX||{};
 ...
 async function applyEditor(meta,{force=false}={}){
   ...
   const info=q('selectedMediaInfo');if(info){const extra=[meta.album,meta.year,meta.cover?'capa automática':''].filter(Boolean).join(' · ');info.textContent=`✓ R8 automático: ${meta.artist||'Artista'} — ${meta.title||'Faixa'}${extra?' · '+extra:''}`}
   return meta;
 }
 async function fillEditorFromFile(file,opts={}){
   const info=document.getElementById('selectedMediaInfo');if(info)info.textContent=`R8 analisando MP3: ${file?.name||'arquivo'}…`;
   try{
 ...
 window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['music-smart-metadata']='NOVA-R8-MP3-AUTO-FIX';
})();
