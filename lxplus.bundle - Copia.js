...
async function openMusicProvider(){if(!$('musicDock')?.classList.contains('external-provider'))return false;LX.toast('A fonte está integrada ao player da LX Music.');return false}
...
async function setupMusicEmbed(desc,autoplay=false){
 ...
 dock.classList.add('external-provider','provider-transport');dock.classList.remove('provider-open');panel.classList.remove('hidden');panel.setAttribute('aria-hidden','true');toggle?.classList.add('hidden');
 ...
 if(frame){
  ...
  if(kind==='youtube'){
   ...
   const join=src.includes('?')?'&':'?';if(!/enablejsapi=1/.test(src))src+=`${join}enablejsapi=1`;if(!/[?&]playsinline=1/.test(src))src+=`${src.includes('?')?'&':'?'}playsinline=1`;if(!/[?&]controls=/.test(src))src+=`&controls=0`;if(!/[?&]iv_load_policy=/.test(src))src+=`&iv_load_policy=3`;if(!/[?&]fs=/.test(src))src+=`&fs=0`;if(/^https?:$/.test(location.protocol)){if(!/[?&]origin=/.test(src))src+=`${src.includes('?')?'&':'?'}origin=${encodeURIComponent(location.origin)}`;if(!/[?&]widget_referrer=/.test(src))src+=`&widget_referrer=${encodeURIComponent(location.href)}`}src+=`${src.includes('?')?'&':'?'}autoplay=${autoplay?1:0}`;
...
function music(id,index=0,autoplay=true){
 ...
 if(autoplay&&cached&&a&&!isExternalMusicTrack(chosen)){
   try{
     ...
     $('musicTitle').textContent=chosen.title||'Faixa';$('musicArtist').textContent=chosen.artist||'LX Music';syncMusicProviderBrand(chosen);LX.artwork.set($('musicCover'),chosen,content);$('musicDock')?.style.setProperty('--music-cover',LX.artwork.safeBackground(chosen,content));setMusicStatus('Faixa completa','ready');syncMusicMediaSession(chosen);updateMusicUI();
...
async function loadTrack(autoplay=false){
 ...
 $('musicTitle').textContent=t.title||'Faixa';$('musicArtist').textContent=t.artist||'LX Music';syncMusicProviderBrand(t);LX.artwork.set($('musicCover'),t,content);dock?.style.setProperty('--music-cover',LX.artwork.safeBackground(t,content));$('musicProgress').value=0;$('musicTime').textContent='0:00';$('musicDuration').textContent=LX.fmt(t.duration||0);if($('musicProviderLabel'))$('musicProviderLabel').textContent='LX Music';syncMusicMediaSession(t);syncMusicCardState();document.dispatchEvent(new CustomEvent('lx:music-changed',{detail:{item:t,index:state.musicIndex,provider:'LX Music'}}));
 const refs=[t.authorizedAudioUrl,t.authorizedStreamUrl,t.authorizedAudioKey,t.fullMediaKey,t.audioKey,t.audioUrl,t.mediaKey,t.url,t.src,content.authorizedAudioUrl,content.authorizedStreamUrl,content.authorizedAudioKey,content.fullMediaKey,content.audioKey,content.audioUrl,content.mediaKey].filter(Boolean);let src=null,embed=null,sourceRef=null;
 try{
  ...
  if(!src&&embed){await setupMusicEmbed(embed,autoplay);if(ticket!==musicLoadTicket)return false;setMusicStatus('Faixa completa','ready');syncMusicProviderBrand(t);syncMusicMediaSession(t);updateMusicUI();syncMusicCardState();return true}
...
(()=>{
 ...
 window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['music-smart-metadata']='31.9';
})();


/* ===== LX Plus v31.9 · branded audio transport + background media controls ===== */
(()=>{
 'use strict';
 ...
 window.__LX_MODULES=window.__LX_MODULES||{};window.__LX_MODULES['v31.9-smart-audio']='31.9';
})();
